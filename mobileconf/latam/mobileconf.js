...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Error de confirmación',
					'Ha habido un problema al realizar esa acción. Por favor, intenta realizar tu solicitud de nuevo más tarde.',
					'Aceptar'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Error de confirmación',
					'Ha habido un problema al cancelar esa confirmación. Por favor, intenta realizar tu solicitud de nuevo más tarde.',
					'Aceptar'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Error de confirmación',
							typeof responseData.message != 'undefined' ? responseData.message : 'Ha habido un problema al cargar los detalles de esa confirmación. Por favor, intenta realizar tu solicitud de nuevo más tarde.',
							'Aceptar'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Error de confirmación',
						'Ha habido un problema al cargar los detalles de esa confirmación. Por favor, intenta realizar tu solicitud de nuevo más tarde.',
						'Aceptar'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Error de confirmación',
				'La aplicación móvil no puede cargar los detalles de esta confirmación. Reinicia tu dispositivo e inténtalo de nuevo más tarde. Si aún sigues teniendo problemas, esto puede deberse a que se ha configurado otro dispositivo para que proporcione los códigos de Steam Guard de tu cuenta, lo que significaría que el autenticador de este dispositivo ya no es válido.',
				'Aceptar'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Cancelar selección' );
			$btnAccept.text( 'Confirmar selección' );
