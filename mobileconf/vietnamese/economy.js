...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Hiển thị thêm' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Vật phẩm chưa xác định ' + itemid, descriptions: [], fraudwarnings: [ 'Không thể thu thập thông tin về vật phẩm này.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Loại bỏ khỏi phiên trao đổi' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Thêm để trao đổi' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Toàn bộ vật phẩm #{appname}');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Kho đồ sau đang không khả dụng tại thời điểm này. Xin vui lòng thử lại sau.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"B\u1ea1n c\u00f3 th\u1ec3 nh\u1eadn ch\u00fang t\u1eeb nh\u1eefng \u0111\u1ed3 r\u01a1i mi\u1ec5n ph\u00ed trong game, C\u1eeda h\u00e0ng Mann Co. trong game, ho\u1eb7c trao \u0111\u1ed5i ch\u00fang v\u1edbi b\u1ea1n b\u00e8."},"620":{"howtoget":"B\u1ea1n c\u00f3 th\u1ec3 nh\u1eadn \u0111\u01b0\u1ee3c ch\u00fang t\u1eeb c\u1eeda h\u00e0ng trong tr\u00f2 ch\u01a1i Portal-2 ho\u1eb7c trao \u0111\u1ed5i ch\u00fang v\u1edbi b\u1ea1n b\u00e8."},"753":{"1":{"howtoget":"B\u1ea1n c\u00f3 th\u1ec3 nh\u1eadn \u0111\u01b0\u1ee3c th\u00eam b\u1ea3n sao c\u00e1c tr\u00f2 ch\u01a1i trong nh\u1eefng d\u1ecbp \u0111\u1eb7c bi\u1ec7t, ho\u1eb7c mua tr\u00f2 ch\u01a1i t\u1eeb c\u1eeda h\u00e0ng Steam v\u00e0 ch\u1ecdn \"Mua \u0111\u1ec3 l\u00e0m qu\u00e0\" l\u00fac thanh to\u00e1n."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> từ #{contextname}');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Số lượng này là riêng tư và chỉ hiện cho mình bạn.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s ngọc'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Hiển thị trong chợ cộng đồng' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Đang xử lý...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Bắt đầu từ: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Hiện không có danh mục nào cho sản phẩm này.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = 'Đã bán được %1$s trong 24 giờ qua';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Giá trung vị: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Số lượng: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Bán' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Bạn không thể bán hàng trên chợ cộng đồng cho đến khi <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">thêm tiền vào ví Steam của mình</a> hoặc thực hiện mua hàng trên cửa hàng Steam và cung cấp địa chỉ thanh toán.',
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Rao bán một vật phẩm' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Bạn phải đồng ý với các điều khoản trong Điều khoản người đăng ký Steam để bán vật phẩm này.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Bạn phải nhập số lượng hợp lệ.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Giá tiền được nhập cao hơn giá tối đa của %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Bạn phải nhập giá trị hợp lệ.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Xác nhận giá bán' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Để đăng vật phẩm này vào chợ cộng đồng, bạn phải xác thực vật bán trong ứng dụng Steam di động của mình. Bạn có thể xác thực bằng cách chạy ứng dụng và điều hướng đến trang "Xác nhận" từ trình đơn.' +
							'<br><br>' +
							'Nếu bạn không thấy lựa chọn xác nhận tại màn hình chính của ứng dụng, hãy đảm bảo bạn đang dùng phiên bản mới nhất của ứng dụng.';

				}
				else
				{
					strText = 'Để bán vật phẩm này trên chợ cộng đồng, bạn phải hoàn tất thêm bước xác minh. Một email đã được gửi tới địa chỉ của bạn (kết thúc là "%s") với các hướng dẫn bổ sung.'.replace( /%s/, transport.responseJSON.email_domain );
				}

				ShowAlertDialog(
						'Cần thêm bước xác nhận',
						strText
				);
			}
			else
		...
		{
			this.DisplayError( 'Đã có vấn đề phát sinh khi bày bán vật phẩm của bạn. Xin vui lòng làm mới trang và thử lại.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Đã có vấn đề phát sinh khi lưu lựa chọn của bạn, xin vui lòng thử lại sau.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Giá bán trung vị', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>Đã bán %d'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Sử dụng mẫu này để báo cáo %s tới đội hỗ trợ Steam về hành vi lừa đảo.<br><br>Xin hãy cung cấp một đoạn mô tả tóm tắt:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Tố cáo lừa đảo', $Dialog, 'Tố cáo lừa đảo').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Tố cáo lừa đảo', 'Cần một đoạn mô tả.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Cảm ơn bạn!', 'Tố cáo của bạn đã được gửi và sẽ được xem xét bởi đội hỗ trợ Steam.' );
		}).fail( function() {
			ShowAlertDialog( 'Tố cáo lừa đảo', 'Đã có vấn đề phát sinh khi lưu tố cáo của bạn. Xin vui lòng thử lại sau.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Hoãn trao đổi và mua bán là gì?</div>' );
	description.append( '<div class="escrow_explanation_a">Phiên hoãn là khoảng thời gian mà vật phẩm sẽ bị tạm giữ bởi Steam trước khi hoàn tất giao dịch được hoàn tất hay gửi liệt kê mua bán đồ.</div>' );

	description.append( '<div class="escrow_explanation_q">Hoãn trao đổi giúp bảo vệ vật phẩm của bạn</div>' );
	description.append( '<div class="escrow_explanation_a">Tài khoản Steam của bạn rất giá trị, đặc biệt nếu bạn có vật phẩm quý giá. Nếu bạn chưa bảo vệ tài khoản bằng một thiết bị khác (bộ xác thực Steam Guard di động), giao dịch của bạn sẽ tạm thời bị giữ để cho bạn thời gian tìm hiểu xem tài khoản của bạn có bị xâm nhập hay không và ngăn cản vật phẩm rời tài khoản của bạn.</div>' );

	description.append( '<div class="escrow_explanation_q">Một thời gian chờ để phát hiện và ngăn chặn việc ăn trộm vật phẩm</div>' );
	description.append( '<div class="escrow_explanation_a">Nếu một người dùng có ý định trao đổi hoặc đem bán các vật phẩm mà tài khoản của người đó không được bảo vệ bởi bộ xác thực di động trong 7 ngày gần đây:<br><span style="color: #5aa9d6">Trao đổi:</span> vật phẩm đã được cả hai bên xác nhận trao đổi sẽ bị Steam giữ lại trong vòng 15 ngày <br><span style="color: #5aa9d6">Giao dịch chợ:</span> các vật phẩm đem bán sẽ bị Steam giữ lại trước khi đăng lên chợ trong vòng 15 ngày <br><br>Điều này cho phép chủ nhân thực sự của tài khoản có thời gian để hủy bỏ mọi giao dịch mà họ không cho phép. <br><br>Việc hủy bỏ bất kỳ trao đổi nào đang chờ hoặc đang bị Steam tạm hoãn sẽ khởi động một lệnh đóng băng trao đổi trên tài khoản của bạn để ngăn cản những hành vi trao đổi vật phẩm trái phép. Hủy liệt kê bán đồ trên chợ không gây ra bất cứ khoảng thời gian đóng băng nào.</div>' );

	description.append( '<div class="escrow_explanation_q">Xóa bỏ việc hoãn trao đổi</div>' );
	description.append( '<div class="escrow_explanation_a">Sử dụng bộ xác thực di động bảo đảm rằng chỉ bạn mới có thể trao đổi vật phẩm nhanh chóng và an toàn, vì vậy việc hoãn trao đổi trở nên không cần thiết. Tăng cường bảo mật tài khoản của bạn bằng cách tải <a href="https://store.steampowered.com/mobile/" target="_blank">Bộ xác thực Steam Guard di động</a> cho thiết bị iOS và Android.</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Đọc bài viết tại Blog Steam về bảo mật tài khoản</a> để đọc giải thích cặn kẽ nguyên nhân chúng tôi quyết định áp dụng việc hoãn trao đổi.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Hoãn trao đổi và mua bán',
			description,
			'Tìm hiểu thêm về bộ xác thực Steam Guard di động',
			'Đóng'
		);
