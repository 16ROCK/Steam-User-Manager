...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Lỗi xác thực',
					'Đã có vấn đề phát sinh khi thực hiện hành động đó. Xin vui lòng thử lại sau.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Lỗi xác thực',
					'Đã có vấn đề phát sinh khi hủy xác thực đó. Xin vui lòng thử lại sau.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Lỗi xác thực',
							typeof responseData.message != 'undefined' ? responseData.message : 'Đã có vấn đề phát sinh khi tải chi tiết cho xác thực đó. Xin vui lòng thử lại sau.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Lỗi xác thực',
						'Đã có vấn đề phát sinh khi tải chi tiết cho xác thực đó. Xin vui lòng thử lại sau.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Lỗi xác thực',
				'Ứng dụng di động hiện không thể tải được thông tin của xác thực. Vui lòng khởi động lại thiết bị của bạn và thử lại yêu cầu lại sau. Nếu bạn vẫn gặp phải vấn đề, nó có thể do một thiết bị khác đã được cài đặt để cung cấp mã Steam Guard cho tài khoản của bạn, có nghĩa là bộ xác nhận trên thiết bị này không còn hợp lệ.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Hủy đã chọn' );
			$btnAccept.text( 'Xác nhận đã chọn' );
