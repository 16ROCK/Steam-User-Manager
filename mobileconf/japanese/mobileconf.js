...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'確認エラー',
					'この操作を実行する際に問題が生じました。後でもう一度お試しください。',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'確認エラー',
					'この認証をキャンセルする際に問題が生じました。後でもう一度お試しください。',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'確認エラー',
							typeof responseData.message != 'undefined' ? responseData.message : 'この認証に関する詳細の読込中に問題が生じました。後でもう一度お試しください。',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'確認エラー',
						'この認証に関する詳細の読込中に問題が生じました。後でもう一度お試しください。',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'確認エラー',
				'モバイルアプリは、その確認の詳細を読み込むことができません。デバイスを再起動してから、もう一度リクエストをお試しください。問題が続く場合は、アカウントに Steam ガードコードを提供するための別のデバイスが設定されている可能性があります。つまり、このデバイスの認証機器は無効になっていることを意味します。　',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( '選択をキャンセル' );
			$btnAccept.text( '選択を確定' );
