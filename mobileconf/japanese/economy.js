...
var CInventory = Class.create( {
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: '不明のアイテム ' + itemid, descriptions: [], fraudwarnings: [ 'このアイテムについての情報は取得できませんでした。' ] };
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'すべての #{appname} アイテム');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'このインベントリは現在使用できません。後でもう一度お試しください。' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"\u7121\u6599\u30b2\u30fc\u30e0\u5185\u30a2\u30a4\u30c6\u30e0\u30c9\u30ed\u30c3\u30d7\u3001Mann Co. Store\u3001\u307e\u305f\u306f\u30d5\u30ec\u30f3\u30c9\u3068\u306e\u30c8\u30ec\u30fc\u30c9\u3067\u5165\u624b\u53ef\u80fd\u3067\u3059\u3002"},"620":{"howtoget":"Portal 2 \u30b2\u30fc\u30e0\u5185\u306e\u30b9\u30c8\u30a2\u3067\u8cfc\u5165\u3059\u308b\u304b\u3001\u30d5\u30ec\u30f3\u30c9\u3068\u306e\u30c8\u30ec\u30fc\u30c9\u3067\u5165\u624b\u53ef\u80fd\u3067\u3059\u3002"},"753":{"1":{"howtoget":"\u7279\u5225\u4f01\u753b\u3084\u3001Steam Store \u3067\u306e\u30c1\u30a7\u30c3\u30af\u30a2\u30a6\u30c8\u6642\u300c\u30ae\u30d5\u30c8\u3068\u3057\u3066\u8cfc\u5165\u300d\u3059\u308b\u4e8b\u3067\u30b2\u30fc\u30e0\u306e\u8ffd\u52a0\u30b3\u30d4\u30fc\u3092\u5165\u624b\u3059\u308b\u3053\u3068\u304c\u3067\u304d\u307e\u3059\u3002"}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '#{contextname} から <span style="#{currencystyle}">#{amount}</span>');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'この額は公開されず、あなたにしか見えません。' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s ジェム'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'コミュニティマーケット内で表示' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="処理中...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += '開始価格： ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'このアイテムの現在利用可能な出品は存在しません。' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '24時間内に %1$s 個販売';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += '中間価格： ' + transport.responseJSON.median_price + '<br>';
							strInfo += '個数: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', '売却' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'コミュニティマーケットでアイテムを販売するには、お客様の <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top"> Steam ウォレットにクレジットをチャージする</a>か、Steam ストアでアイテムを購入して請求先住所を指定してください。',
			'アイテムを販売できません'
		);
	}
	else
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'アイテムを売却対象として選択' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'このアイテムを販売するには、Steam 利用規約に同意していただく必要があります。' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( '有効な数量を入力する必要があります。' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' 入力した値が最大価格の %1$s を超えています。'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( '有効な価格を入力してください。' );
			return;
		}
		...
		$('market_sell_dialog_title').update( '売却価格を確認する' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'このコミュニティマーケット上でこのアイテムを出品するには、Steam モバイルアプリで確認を行う必要があります。アプリを起動し、メニューから確認ページを開けば確認を行うことができます。' +
							'<br><br>' +
							'アプリのメインメニューに確認オプションが見つからなければ、アプリが最新バージョンかどうか確認してください。';

				}
				else
				{
					strText = 'このアイテムをコミュニティマーケットに出品するには、追加の確認ステップの完了が必要です。メールアドレス（末尾が "%s"）へ追加のインストラクションが送信されました。'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'アイテムの出品中に問題が発生しました。ページを更新して、もう一度実行してください。' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'あなたの選択の保存中にエラーが発生しました。後でもう一度お試しください。' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: '中間販売価格', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d 売れました'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'これを利用して、詐欺の試みで Steam サポートに %s を報告してください。<br><br>簡単な説明をお願いします:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( '詐欺を報告', $Dialog, '詐欺を報告').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( '詐欺を報告', '説明は必須です。' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'ありがとうございます！', '報告は提出されました。 Steam サポートが調査します。' );
		}).fail( function() {
			ShowAlertDialog( '詐欺を報告', '報告の保存中にエラーが発生しました。後でもう一度お試しください。' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">トレード／マーケットホールドとは？</div>' );
	description.append( '<div class="escrow_explanation_a">ホールドとは、トレードが完了するかマーケットへ出品される前に Steam がアイテムを一時的に預かる期間のことです。</div>' );

	description.append( '<div class="escrow_explanation_q">アイテムホールドはあなたのアイテムを守るのに役立ちます</div>' );
	description.append( '<div class="escrow_explanation_a">Steam アカウントには価値があり、盗む価値のあるアイテムを持ったアカウントは特に高い価値を持ちます。ご利用のアカウントを物理的なデバイス（Steam ガードモバイル認証機器）で保護していない場合、トレードホールドが時間を稼ぐことによって不正にアクセスされたことに気づき、アイテムの盗難を防ぐことができます。</div>' );

	description.append( '<div class="escrow_explanation_q">アイテムの盗難防止のための猶予</div>' );
	description.append( '<div class="escrow_explanation_a">アイテムをトレードで渡す、あるいはマーケットで売却するユーザーのアカウントがモバイル認証機器によって過去 7 日間保護されていなかった場合: <br><span style="color: #5aa9d6">トレード:</span> アイテムの受け渡しが最長 15 日間ホールドされます<br><span style="color: #5aa9d6">マーケット:</span> 売却のための出品が最長 15 日間ホールドされます<br><br>これによって、ユーザーは自分が許可していない保留中の取引をキャンセルする時間を得られます。<br><br>保留中、またはホールド中のトレードがキャンセルされると、その後、無許可のトレードによるアイテム流出を防ぐため、アカウントにトレードクールダウンが適用されます。マーケットへの出品をキャンセルしてもクールダウンは適用されません。</div>' );

	description.append( '<div class="escrow_explanation_q">アイテムホールドを不要にするには</div>' );
	description.append( '<div class="escrow_explanation_a">モバイル認証機器があれば、本人以外によるアクセスがブロックされることでアカウントの安全性が高まり、アイテムをホールドする必要がなくなります。iOS あるいはアンドロイドデバイスに<a href="https://store.steampowered.com/mobile/" target="_blank"> Steam ガードモバイル認証</a>を導入して、安全かつ迅速なトレードや販売をお楽しみください。</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">アカウントセキュリティに関する Steam ブログの記事</a>をお読みください。ホールド機能の実装理由に関して解説されています。</div>' );
