...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Chyba potvrzení',
					'Při provádění této akce došlo k chybě. Zkuste to znovu později.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Chyba potvrzení',
					'Při rušení potvrzení došlo k chybě. Zkuste to znovu později.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Chyba potvrzení',
							typeof responseData.message != 'undefined' ? responseData.message : 'Při načítání podrobností tohoto potvrzení došlo k chybě. Zkuste to znovu později.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Chyba potvrzení',
						'Při načítání podrobností tohoto potvrzení došlo k chybě. Zkuste to znovu později.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Chyba potvrzení',
				'Mobilní aplikace nemůže načíst podrobnosti tohoto potvrzení. Restartujte prosím své zařízení a znovu proveďte žádost. Pokud nebude potvrzení fungovat ani poté, je možné, že autentifikátor na tomto zařízení již není platný a kódy služby Steam Guard pro Váš účet nyní poskytuje jiné zařízení.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Zrušit vybrané' );
			$btnAccept.text( 'Potvrdit vybrané' );
