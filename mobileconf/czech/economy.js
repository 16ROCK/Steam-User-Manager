			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Odstranit z obchodu' );
					}
					else
					{
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Všechny #{appname} položky');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Inventář je momentálně nedostupný. Zkuste to prosím později.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Lze je z\u00edskat zdarma n\u00e1lezem ve h\u0159e, z obchodu Mann Co., nebo v\u00fdm\u011bnou s p\u0159\u00e1teli."},"620":{"howtoget":"Lze je z\u00edskat bu\u010f z obchodu ve h\u0159e Portal 2, \u010di v\u00fdm\u011bnou s p\u0159\u00e1teli."},"753":{"1":{"howtoget":"Extra kopie her m\u016f\u017eete z\u00edskat b\u011bhem speci\u00e1ln\u00edch akc\u00ed nebo koupen\u00edm hry ze Steam Obchodu a vybr\u00e1n\u00edm mo\u017enosti \"Koupit jako d\u00e1rek\" p\u0159ed potvrzen\u00edm."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Toto množství je osobní a zobrazuje se pouze vám.' );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Zpracovávání...">' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Nemůžete prodávat položky na komunitním trhu, dokud <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">nepřidáte prostředky do své peněženky služby Steam</a> nebo neprovedete nákup v obchodě služby Steam a neuvedete svou fakturační adresu.',
			'Položku nelze prodat'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Nabídnutí položky k prodeji' );
	...
	OnAccept: function( event ) {
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Musíte zadat platné množství.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Zadaná cena přesahuje maximální cenu %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Je nutné zadat platnou cenu.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Potvrzení prodejní ceny' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Aby bylo možné vystavit tuto položku na komunitním trhu, je nutné ji nejdříve ověřit pomocí mobilní aplikace služby Steam. Ověření provedete na stránce „Potvrzení“ v hlavní nabídce aplikace.' +
							'<br><br>' +
							'Pokud v hlavní nabídce mobilní aplikace služby Steam nemůžete nalézt možnost „Potvrzení“, zkontrolujte, zda máte nainstalovanou nejnovější verzi aplikace.';

				}
				else
				{
					strText = 'Vystavení této položky na komunitním trhu musíte nejdříve ověřit. Na Vaši e-mailovou adresu (končící „%s“) byla zaslána zpráva s dalšími instrukcemi.'.replace( /%s/, transport.responseJSON.email_domain );
				}

				ShowAlertDialog(
						'Je vyžadováno potvrzení obchodu',
						strText
				);
			}
			else
		...
		{
			this.DisplayError( 'Při vytváření Vaší nabídky došlo k chybě. Aktualizujte stránku a zkuste to znovu.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Při ukládání výběru došlo k chybě. Zkuste to prosím později.' );
	}
}
...
function ReportTradeScam( steamIDTarget, strPersonaName )
{
	...
	var Modal = ShowConfirmDialog( 'Nahlásit podvod', $Dialog, 'Nahlásit podvod').done( function() {
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			...
			ShowAlertDialog( 'Nahlásit podvod', 'Při ukládání Vašeho nahlášení došlo k chybě. Zkuste to prosím později znovu.' );
		});
	} );
