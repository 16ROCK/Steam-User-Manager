...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Dahasını göster' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Bilinmeyen Eşya ' + itemid, descriptions: [], fraudwarnings: [ 'Bu eşya hakkındaki bilgiye ulaşılamadı.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Takastan kaldır' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Takasa ekle' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Tüm #{appname} Öğeleri');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Envantere şu an ulaşılamıyor. Lütfen daha sonra tekrar deneyin.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Onlar\u0131 oyun i\u00e7inde \u00fccretsiz bulabilir, Mann Co. Ma\u011fazas\u0131ndan sat\u0131n alabilir ya da arkada\u015flar\u0131n\u0131zdan takas ile elde edebilirsiniz."},"620":{"howtoget":"Onlar\u0131 Portal 2 oyun i\u00e7i ma\u011fazadan alabilir veya arkada\u015flar\u0131n\u0131zdan takas ile elde edebilirsiniz."},"753":{"1":{"howtoget":"Ekstra oyun kopyalar\u0131n\u0131 promosyonlar s\u0131ras\u0131nda veya Steam Ma\u011fazas\u0131'ndan bir oyun sat\u0131n al\u0131rken \"Hediye Olarak Sat\u0131n Al\" se\u00e7ene\u011fiyle elde edebilirsiniz."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '#{contextname} adlı kullanıcıdan <span style="#{currencystyle}">#{amount}</span>');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Bu değer gizlidir ve sadece sizin tarafınızdan görülebilir.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s Cevher'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Topluluk Pazarında Görüntüle' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Çalışıyor...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'En düşük fiyat: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Bu öğe için mevcut bir listeleme bulunmuyor.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = 'Son 24 saatte %1$s adet satıldı';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Medyan fiyat: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Miktar: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Sat' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'<a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">Steam Cüzdanınıza para eklemeden</a> veya Steam mağazasında bir satın alma işlemi gerçekleştirip fatura adresinizi vermeden Topluluk Pazarında öğe satamazsınız.',
			'Eşya satılamıyor'
		);
	}
	else
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Bir öğeyi satışa çıkarın' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Bu eşyayı satmak için Steam Abonelik Sözleşmesi şartlarını kabul etmelisiniz.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Geçerli bir miktar girmelisiniz.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Girilen fiyat %1$s sınırını aşıyor.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Geçerli bir fiyat girmelisiniz.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Satış fiyatını onaylayın' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Topluluk Pazarı\'nda bu öğeyi listelemek için Steam Mobil uygulamasından listelemenizi onaylamanız gerekmektedir. Uygulamayı başlatıp, menüden Onaylamalar sayfasına giderek bunu onaylayabilirsiniz.' +
							'<br><br>' +
							'Eğer Onaylamalar seçeneğini uygulamanın ana menüsünde göremiyorsanız, uygulamanın son sürümüne sahip olduğunuzdan emin olun.';

				}
				else
				{
					strText = 'Topluluk Pazarında bu eşyayı listeleyebilmek için ek bir onaylama aşamasını tamamlamanız gerekmektedir. Ek yönergeleri içeren bir e-posta (sonu "%s" ile biten), adresinize gönderildi.'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'Eşyanız listelenirken bir sorun oluştu. Sayfayı yenileyin ve tekrar deneyin.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Seçiminiz kaydedilirken bir hata meydana geldi, lütfen tekrar deneyin.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Medyan Satış Fiyatları', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d tane satıldı'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( '%s isimli kullanıcıyı dolandırıcılığa teşebbüsten Steam Destek\'e bildirmek için bunu kullanın.<br><br>Lütfen açıklamanızı kısa tutun.'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Dolandırıcılık Bildir', $Dialog, 'Dolandırıcılık Bildir').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Dolandırıcılık Bildir', 'Bir açıklama gereklidir.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Teşekkür Ederiz!', 'Raporunuz gönderildi ve Steam Destek tarafından incelenecek.' );
		}).fail( function() {
			ShowAlertDialog( 'Dolandırıcılık Bildir', 'Raporunuzu kaydetmede bir hata oluştu. Lütfen daha sonra tekrar deneyin.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Takas ya da pazar bekletmesi nedir?</div>' );
	description.append( '<div class="escrow_explanation_a">Bekletme, pazar listelemeleri veya takas işlemi tamamlanmadan önce öğenizin Steam tarafından tutulduğu süreçtir.</div>' );

	description.append( '<div class="escrow_explanation_q">Öğe bekletmeleri öğelerinizi korumanıza yardımcı olur</div>' );
	description.append( '<div class="escrow_explanation_a">Steam hesapları değerlidir, özellikle de çalınmaya değer öğeler barındırıyorsa. Eğer hesabınızı fiziksel (Steam Guard Mobil Kimlik Doğrulayıcı) bir cihaz ile korumadıysanız, takas bekletmesi, size hesabınızın tehlikede olup olmadığını anlamanıza olanak tanıyacak ve öğelerinizin hesabınızdan gitmesine engel olacaktır.</div>' );

	description.append( '<div class="escrow_explanation_q">Öğe hırsızlığını durdurabilmek ve yakalayabilmek için gecikme</div>' );
	description.append( '<div class="escrow_explanation_a"></div>' );

	description.append( '<div class="escrow_explanation_q">Öğe bekletmeleri gerekliliğini kaldırın</div>' );
	description.append( '<div class="escrow_explanation_a">Mobil Kimlik Doğrulayıcı kullanmak, kendi öğelerinizi sadece sizin çabuk ve güvenli bir şekilde takas edebilmenizi sağlar ve böylece takas bekletmesine artık gerek kalmaz. iOS ve Android cihazlar için olan <a href="https://store.steampowered.com/mobile/" target="_blank">Steam Guard Mobil Kimlik Doğrulayıcıyı</a> edinerek hesabınızın güvenliğini artırın.</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Hesap güvenliği hakkındaki Steam blog gönderimizi okuyarak</a>, neden bekletme işlemlerini uyguladığımızı detaylı bir şekilde öğrenebilirsiniz.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Takas ve Pazar Beklemeleri',
			description,
			'Steam Guard mobil kimlik doğrulayıcı hakkında daha fazlasını öğrenin',
			'Kapat'
		);
