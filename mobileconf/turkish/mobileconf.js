...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Onaylama Hatası',
					'Bu eylemi gerçekleştirirken bir sorun oluştu. Lütfen talebinizi daha sonra tekrar deneyiniz.',
					'Tamam'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Onaylama Hatası',
					'Onaylamayı iptal ederken bir sorun oluştu. Lütfen talebinizi daha sonra tekrar deneyin.',
					'Tamam'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Onaylama Hatası',
							typeof responseData.message != 'undefined' ? responseData.message : 'Bu onaylama için ayrıntıları yüklerken bir sorun oluştu. Lütfen talebinizi daha sonra tekrar deneyin.',
							'Tamam'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Onaylama Hatası',
						'Bu onaylama için ayrıntıları yüklerken bir sorun oluştu. Lütfen talebinizi daha sonra tekrar deneyin.',
						'Tamam'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Onaylama Hatası',
				'Mobil uygulama o onaylama için detayları yükleyemedi. Lütfen cihazınızı yeniden başlatın ve isteğinizi tekrar deneyin. Eğer hâlâ bir sorunla karşılaşıyorsanız bunun sebebi hesabınıza Steam Guard kodlarını sağlaması için başka bir cihazın ayarlanmış olması ve dolayısıyla elinizdeki mevcut cihazda yer alan kimlik doğrulayıcının artık geçerli olmaması olabilir.',
				'Tamam'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Seçileni İptal Et' );
			$btnAccept.text( 'Seçileni Onayla' );
