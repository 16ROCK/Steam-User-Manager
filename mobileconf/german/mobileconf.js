...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Bestätigungsfehler',
					'Beim Ausführen dieser Aktion ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Bestätigungsfehler',
					'Beim Abbruch der Bestätigung ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Bestätigungsfehler',
							typeof responseData.message != 'undefined' ? responseData.message : 'Beim Laden der Details für diese Bestätigung ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Bestätigungsfehler',
						'Beim Laden der Details für diese Bestätigung ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Bestätigungsfehler',
				'Die Steam-Mobile-App kann keine Details dieser Bestätigung abrufen. Bitte starten Sie Ihr Gerät neu und versuchen Sie es später erneut. Wenn Sie immer noch Probleme haben, könnte es sein, dass ein anderes Gerät als Steam Guard eingerichtet wurde, also der derzeitige Authenfikator dieses Geräts seine Gültigkeit verloren hat.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Ausgewählte abbrechen' );
			$btnAccept.text( 'Ausgewählte bestätigen' );
