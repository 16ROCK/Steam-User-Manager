...
var kStandardTag_Tradable =
{
	...
	category: "misc",
	category_name: 'Verschiedene'
};

var kStandardTag_Untradable =
{
	name: 'Nicht handelbar',
...
var CInventory = Class.create( {
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Unbekannter Gegenstand ' + itemid, descriptions: [], fraudwarnings: [ 'Abrufen von Informationen über diesen Gegenstand fehlgeschlagen.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Vom Handel entfernen' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Zum Handel hinzufügen' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Alle Gegenstände aus #{appname}');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Dieses Inventar ist momentan nicht erreichbar. Bitte versuchen Sie es später erneut.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Sie erhalten sie durch kostenlose Funde im Spiel, aus dem im Spiel integrierten Mann Co. Store oder durch Handel mit Freunden."},"620":{"howtoget":"Sie erhalten sie aus dem in Portal 2 integriertem Shop oder durch Handel mit Freunden."},"753":{"1":{"howtoget":"Sie k\u00f6nnen zus\u00e4tzliche Kopien eines Spiels als Werbeangebot erhalten oder ein Spiel im Steam Store \"als Geschenk kaufen\"."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> von #{contextname}');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Dieser Betrag ist privat und wird nur Ihnen angezeigt.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s Edelsteine'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Auf dem Communitymarkt anzeigen' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Vorgang wird bearbeitet …">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Ab: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Für diesen Gegenstand gibt es derzeit keine Angebote.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '%1$s in den letzten 24 Stunden verkauft';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Medianpreis: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Anzahl: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Verkaufen' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Sie können erst Artikel über den Communitymarkt verkaufen, wenn Sie <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">Ihr Steam-Guthaben aufladen</a> oder einen Kauf im Steam-Shop tätigen und Ihre Rechnungsadresse angeben.',
			'Gegenstand kann nicht verkauft werden'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Einen Artikel zum Verkauf anbieten' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Um diesen Gegenstand verkaufen zu können, müssen Sie den Bedingungen des Steam-Nutzungsvertrags zustimmen.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Sie müssen eine gültige Menge eingeben.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Der eingegebene Preis übersteigt den Höchstpreis von %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Sie müssen einen gültigen Preis eingeben.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Verkaufspreis bestätigen' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Um diesen Gegenstand auf dem Communitymarkt anzubieten, müssen Sie dies zunächst in Ihrer Steam-Mobile-App bestätigen. Sie können es bestätigen, wenn Sie die App starten und im Menü zur Bestätigungsseite navigieren.' +
							'<br><br>' +
							'Falls Sie den Menüpunkt "Bestätigungen" im Hauptmenü der App nicht finden können, stellen Sie bitte sicher, dass Sie die neueste Version der App installiert haben.';

				}
				else
				{
					strText = 'Um diesen Gegenstand auf dem Communitymarkt anzubieten, müssen Sie zunächst einen zusätzlichen Verifizierungsschritt abschließen. Eine E-Mail mit zusätzlichen Informationen wurde an Ihre E-Mail-Adresse (endet auf "%s") gesendet.'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'Beim Erstellen des Angebots für Ihren Artikel ist ein Problem aufgetreten. Aktualisieren Sie die Seite, und versuchen Sie es erneut.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Beim Speichern Ihrer Auswahl ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Median der Verkaufspreise', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d verkauft'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Verwenden Sie dieses Formular, um %s wegen Betrugsversuchs an den Steam-Support zu melden.<br><br>Bitte geben Sie eine kurze Beschreibung an:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Betrug melden', $Dialog, 'Betrug melden').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Betrug melden', 'Eine Beschreibung ist erforderlich.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Vielen Dank!', 'Ihre Meldung wurde übermittelt und wird vom Steam-Support überprüft.' );
		}).fail( function() {
			ShowAlertDialog( 'Betrug melden', 'Beim Speichern Ihrer Meldung ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Was sind zurückgehaltene Handelsangebote oder Markttransaktionen?</div>' );
	description.append( '<div class="escrow_explanation_a">Es handelt sich dabei um einen Zeitraum, in dem Gegenstände von Steam zurückgehalten werden, bevor ein Handel abgeschlossen oder eine Markttransaktion gelistet wird.</div>' );

	description.append( '<div class="escrow_explanation_q">Das Zurückhalten von Gegenständen hilft, diese zu schützen</div>' );
	description.append( '<div class="escrow_explanation_a">Steam-Accounts sind wertvoll, vor allem, wenn Sie wertvolle Gegenstände besitzen. Wenn Sie Ihren Account nicht mit einem physischen Gerät (dem Steam-Mobile-Authentifikator) geschützt haben, geben Ihnen zurückgehaltene Handelsangebote die Möglichkeit, zu überprüfen, ob Ihr Account kompromittiert wurde, und Ihre Gegenstände vor unautorisiertem Handel zu beschützen.</div>' );

	description.append( '<div class="escrow_explanation_q">Eine Verzögerung, um Gegenstandsdiebstahl zu entdecken und zu verhindern</div>' );
	description.append( '<div class="escrow_explanation_a"></div>' );

	description.append( '<div class="escrow_explanation_q">Das Zurückhalten von Gegenständen verhindern</div>' );
	description.append( '<div class="escrow_explanation_a">Wenn Sie einen Steam-Mobile-Authentifikator verwenden, stellen Sie damit sicher, dass Sie - und nur Sie - Ihre Gegenstände schnell und sicher handeln und verkaufen können, daher ist das Zurückhalten von Gegenständen dann nicht mehr nötig. Erhöhen Sie Ihre Accountsicherheit, indem Sie sich den <a href="https://store.steampowered.com/mobile/" target="_blank">Steam-Mobile-Authentifikator</a> für iOS- und Android-Geräte holen.</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Lesen Sie den Steam-Blogeintrag über Accountsicherheit</a>, um eine detaillierte Erklärung dazu zu erhalten, warum wir das Zurückhalten von Gegenständen eingeführt haben.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Zurückgehaltene Handelsangebote und Markttransaktionen',
			description,
			'Mehr über den Steam-Mobile-Authentifikator erfahren',
			'Schließen'
		);
	...
	else
	{
		var dialog = ShowAlertDialog(
			'Zurückgehaltene Handelsangebote',
			description
		);
	}
	return false;
