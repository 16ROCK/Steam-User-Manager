...
{
	name: 'Não utilizável no Mercado',
...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Mostrar mais' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Item Desconhecido ' + itemid, descriptions: [], fraudwarnings: [ 'Não foi possível obter informações sobre este item.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Remover da troca' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Adicionar à troca' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Todos os itens (#{appname})');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Este inventário não está disponível neste momento. Por favor, tenta novamente mais tarde.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Podes obt\u00ea-los gratuitamente jogando, comprando-os na Loja Mann Co. ou trocando por eles com amigos."},"620":{"howtoget":"Podes adquiri-los atrav\u00e9s da loja do Portal 2 ou de trocas com amigos."},"753":{"1":{"howtoget":"Podes adquirir c\u00f3pias extra de jogos durante promo\u00e7\u00f5es especiais, ou comprando um jogo na Loja Steam e selecionando \"Comprar como Presente\" no checkout."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Esta quantia é privada e exibida só para ti.' );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Ver no Mercado da Comunidade' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="A processar...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'De momento não há nenhum anúncio disponível para este item.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '%1$s vendido(s) nas últimas 24 horas';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Preço mediano: ' + transport.responseJSON.median_price + '<br>';
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Não podes vender itens no Mercado da Comunidade até <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">adicionares fundos à tua Carteira Steam</a> ou efetuares uma compra na loja Steam e forneceres o teu endereço de cobrança.',
			'Não é possível vender o item'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Coloca um item à venda' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Tens de concordar com os termos do Acordo de Subscrição Steam para vender este item.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Tens de inserir uma quantidade válida.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' O preço introduzido excede o preço máximo de %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Tens de introduzir um preço válido.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Confirma o preço de venda' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Para poderes anunciar este item no Mercado da Comunidade, terás de confirmar esta ação através da aplicação Steam para dispositivos móveis. Podes fazê-lo acedendo à página de Confirmações do menu da aplicação.' +
							'<br><br>' +
							'Se não encontras a opção "Confirmações" no menu principal da aplicação Steam para dispositivos móveis, então certifica-te de que tens a versão mais recente.';

				}
				else
				{
					strText = 'Para poderes anunciar este item no Mercado da Comunidade, tens de completar um passo de confirmação adicional. Um e-mail foi enviado para o teu endereço (que termina em "%s") com instruções adicionais.'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'Ocorreu um problema ao anunciar o teu item. Atualiza a página e tenta novamente.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Ocorreu um problema ao guardar a tua seleção, tenta de novo mais tarde.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Preços medianos de venda', textAlign: 'left' },
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Usa isto para denunciar %s ao Suporte Steam por tentativa de burla. <br><br>Por favor, escreve uma breve descrição:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Denunciar burla', $Dialog, 'Denunciar burla').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Denunciar burla', 'É necessária uma descrição.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Obrigado!', 'A tua denúncia foi enviada e vai ser avaliada pelo Suporte Steam.' );
		}).fail( function() {
			ShowAlertDialog( 'Denunciar burla', 'Ocorreu um problema ao guardar a tua denúncia. Tenta de novo mais tarde.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">O que é uma retenção?</div>' );
	description.append( '<div class="escrow_explanation_a">Uma retenção significa que os itens são retidos pelo Steam durante um certo período de tempo antes de serem trocados ou colocados à venda no Mercado.</div>' );

	description.append( '<div class="escrow_explanation_q">A retenção de itens ajuda a proteger os teus itens</div>' );
	description.append( '<div class="escrow_explanation_a">Porque as contas Steam são valiosas, especialmente se tiverem itens de grande valor. Caso não estejas a proteger a tua conta com um dispositivo físico (o Autenticador Móvel do Steam Guard), uma retenção irá dar-te tempo para descobrir se a tua conta foi acedida por outra pessoa sem a tua autorização e para impedir os teus itens de saírem da tua conta.</div>' );

	description.append( '<div class="escrow_explanation_q">Um adiamento para detetar e impedir o roubo de itens</div>' );
	description.append( '<div class="escrow_explanation_a">Se um utilizador que estiver a trocar itens não tiver a conta dele protegida por um Autenticador Móvel há pelo menos 7 dias: <br><span style="color: #5aa9d6">Trocas:</span> O Steam irá reter os itens de trocas concluídas e a entrega destes será adiada por 15 dias no máximo<br><span style="color: #5aa9d6">Mercado:</span> O Steam irá reter itens, antes de ficarem à venda no Mercado, durante até 15 dias <br><br>Isto dá tempo ao utilizador de cancelar quaisquer transações pendentes não autorizadas. <br><br>Cancelar trocas pendentes ou retidas fará com que a conta fique proibida de fazer trocas temporariamente, de forma a impedir quaisquer futuras tentativas de troca não autorizadas. Isto não acontece ao cancelar a venda de itens no Mercado.</div>' );

	description.append( '<div class="escrow_explanation_q">Remove a necessidade do Steam de reter trocas</div>' );
	description.append( '<div class="escrow_explanation_a">Usar um Autenticador Móvel garante que só tu possas trocar ou vender os teus itens de forma rápida e segura, eliminando assim a necessidade de reter trocas. Aumenta a segurança da tua conta obtendo o <a href="https://store.steampowered.com/mobile/" target="_blank">Autenticador Móvel do Steam Guard</a> para dispositivos Android e iOS.</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Lê este artigo do Steam sobre segurança de contas</a> para uma explicação detalhada sobre as razões que nos levaram a implementar esta funcionalidade.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Retenções de itens em trocas e no Mercado',
			description,
			'Mais informações sobre o Autenticador Móvel do Steam Guard',
			'Fechar'
		);
