...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Mostra di più' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Oggetto sconosciuto ' + itemid, descriptions: [], fraudwarnings: [ 'Impossibile recuperare informazioni riguardo questo oggetto.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Rimuovi dallo scambio' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Aggiungi allo scambio' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Tutti gli oggetti di #{appname}');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Questo inventario è momentaneamente non disponibile. Riprova più tardi.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Puoi trovarli tramite il rilascio degli oggetti gratuitamente durante il gioco, comprandoli dal Negozio Mann Co. o ricevendoli tramite uno scambio con degli amici."},"620":{"howtoget":"Puoi comprarli dal negozio di Portal 2 o riceverli tramite scambio con gli amici."},"753":{"1":{"howtoget":"Puoi ottenere copie extra di giochi durante promozioni speciali, oppure comprando un gioco dal Negozio Steam e selezionando \"Acquista come Dono\" al momento dell'acquisto."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> da #{contextname}');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'L\'importo è privato e visibile solo a te.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s gemme'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Visualizza nel Mercato della Comunità' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="In lavorazione...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Prezzo iniziale: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Non ci sono inserzioni disponibili per questo oggetto.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '%1$s venduti nelle ultime 24 ore';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Prezzo mediano: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Quantità: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Vendi' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Non puoi vendere oggetti nel Mercato della Comunità finché non <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">aggiungi fondi al tuo Portafoglio di Steam</a> o effettui un acquisto nel negozio di Steam e fornisci il tuo indirizzo di fatturazione.',
			'Impossibile vendere l\'oggetto'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Metti in vendita un oggetto' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Devi accettare i termini del Contratto di Sottoscrizione a Steam per vendere questo oggetto.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Devi inserire una quantità valida.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Il prezzo inserito supera il prezzo massimo di %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Devi inserire un prezzo valido.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Conferma prezzo di vendita' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Per pubblicare l\'inserzione nel Mercato della Comunità devi prima verificarla sulla tua app Steam mobile. Per farlo, avvia la app e vai alla pagina delle conferme dal menu.' +
							'<br><br>' +
							'Se non vedi l\'opzione Conferme nel menu principale della app, controlla se hai la versione più aggiornata della app.';

				}
				else
				{
					strText = 'Per mettere in vendita questo oggetto sul Mercato della Comunità devi completare una procedura di verifica. Abbiamo inviato le informazioni necessarie al tuo indirizzo e-mail che termina con "%s".'.replace( /%s/, transport.responseJSON.email_domain );
				}

				ShowAlertDialog(
						'Conferma aggiuntiva necessaria',
						strText
				);
			}
			else
		...
		{
			this.DisplayError( 'Si è verificato un problema durante l\'inserimento del tuo oggetto. Aggiorna la pagina e riprova.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Si è verificato un problema durante il salvataggio della tua selezione, riprova più tardi.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Prezzi di vendita mediani', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d venduti'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Usa questa funzionalità per segnalare %s all\'Assistenza di Steam per un tentativo di truffa.<br><br>Fornisci una breve descrizione:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Segnala una truffa', $Dialog, 'Segnala una truffa').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Segnala una truffa', 'È richiesta una descrizione.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Grazie!', 'La tua segnalazione è stata inviata e verrà esaminata dall\'Assistenza di Steam.' );
		}).fail( function() {
			ShowAlertDialog( 'Segnala una truffa', 'Si è verificato un problema durante il salvataggio della segnalazione. Riprova più tardi.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Cos\'è una sospensione di scambio o di mercato?</div>' );
	description.append( '<div class="escrow_explanation_a">Una sospensione è un periodo di tempo in cui gli oggetti vengono trattenuti da Steam, prima che uno scambio venga completato o prima che una lista di mercato venga pubblicata.</div>' );

	description.append( '<div class="escrow_explanation_q">Le sospensioni sugli oggetti ti aiutano a proteggere i tuoi oggetti</div>' );
	description.append( '<div class="escrow_explanation_a">Gli account di Steam sono preziosi, specialmente se contengono oggetti di un certo valore. Se non hai protetto il tuo account mediante un dispositivo fisico (l\'Autenticatore Mobile di Steam Guard), una sospensione di scambio ti darà il tempo necessario per scoprire se la sicurezza del tuo account è stata compromessa e per evitare che ti vengano sottratti oggetti dal tuo account.</div>' );

	description.append( '<div class="escrow_explanation_q">Una pausa per scoprire e impedire furti di oggetti</div>' );
	description.append( '<div class="escrow_explanation_a">Se un utente che scambia o vende oggetti non ha protetto il proprio account con l\'autenticatore mobile da almeno 7 giorni: <br><span style="color: #5aa9d6">Scambi:</span> Steam posticiperà la consegna degli oggetti ottenuti da uno scambio completato per un massimo di 15 giorni <br><span style="color: #5aa9d6">Mercato:</span> Steam sospenderà le inserzioni per un massimo di 15 giorni prima che vengano pubblicate <br><br>Ciò permetterà all\'utente di avere tutto il tempo necessario per annullare qualsiasi transazione non autorizzata. <br><br>Al fine di evitare qualsiasi ulteriore tentativo non autorizzato di sottrarti degli oggetti, l\'annullamento degli scambi sospesi o in attesa disattiverà temporaneamente la funzionalità degli scambi sul tuo account. Questa disattivazione temporanea non è prevista per l\'annullamento delle inserzioni nel Mercato.</div>' );

	description.append( '<div class="escrow_explanation_q">Evitare la sospensione di oggetti</div>' );
	description.append( '<div class="escrow_explanation_a">Usando un autenticatore mobile potrai assicurarti che soltanto tu potrai scambiare o vendere i tuoi oggetti in modo sicuro e veloce, quindi le sospensioni non saranno più necessarie. Aumenta il livello di sicurezza del tuo account con l\'<a href="https://store.steampowered.com/mobile/" target="_blank">Autenticatore mobile di Steam Guard</a> per dispositivi iOS e Android. </div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Leggi il blog di Steam sulla sicurezza degli account</a> per una spiegazione dettagliata dei motivi per cui abbiamo introdotto le sospensioni.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Sospensioni di scambio e di mercato',
			description,
			'Altre informazioni sull\'Autenticatore Mobile di Steam Guard',
			'Chiudi'
		);
