...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Errore di conferma',
					'Non siamo riusciti a completare questa azione. Riprova più tardi.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Errore di conferma',
					'Non siamo riusciti ad annullare la conferma. Riprova più tardi.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Errore di conferma',
							typeof responseData.message != 'undefined' ? responseData.message : 'Non siamo riusciti a caricare i dettagli della conferma. Riprova più tardi.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Errore di conferma',
						'Non siamo riusciti a caricare i dettagli della conferma. Riprova più tardi.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Errore di conferma',
				'L\'app mobile non riesce a caricare i dettagli di questa conferma. Riavvia il dispositivo e riprova più tardi. Se il problema persiste, è possibile che un altro dispositivo sia stato configurato per fornire i codici di Steam Guard per il tuo account, il che significa che l\'autenticatore su questo dispositivo non è più valido.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Annulla la selezione' );
			$btnAccept.text( 'Conferma la selezione' );
