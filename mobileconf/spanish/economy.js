...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Mostrar más' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Artículo desconocido ' + itemid, descriptions: [], fraudwarnings: [ 'No se ha podido recuperar información de este artículo.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Eliminar del intercambio' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Añadir al intercambio' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Todos los artículos de #{appname}');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Este inventario no se encuentra disponible en estos momentos. Por favor, inténtalo de nuevo más tarde.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Puedes conseguirlos de forma gratuita desde el sistema de obtenci\u00f3n aleatoria del juego, la Tienda Mann Co. o intercambi\u00e1ndolos con tus amigos."},"620":{"howtoget":"Puedes conseguirlos desde la tienda del juego Portal 2 o intercambi\u00e1ndolos con tus amigos."},"753":{"1":{"howtoget":"Puedes conseguir copias extra de juegos durante las promociones especiales o comprando un juego en la tienda de Steam y seleccionando \"Comprar como regalo\" en el momento del pago."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Esta cantidad es privada y solo tú puedes verla.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s gemas'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Ver en el Mercado de la Comunidad' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Procesando...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'A partir de ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'No hay publicaciones disponibles actualmente para este artículo.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '%1$s vendido(s) en las últimas 24 horas';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Precio medio: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Volumen: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Vender' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'No puedes vender artículos en el Mercado de la Comunidad hasta que <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">añadas fondos a tu Cartera de Steam</a> o realices una compra en la tienda de Steam y nos proporciones tu dirección de facturación.',
			'No puedes vender el artículo'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Poner un artículo a la venta' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Para vender este artículo, es necesario aceptar las condiciones del Acuerdo de Suscriptor a Steam.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Debes introducir una cantidad válida.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' El precio introducido supera el precio máximo de %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Debes introducir un precio válido.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Confirmar el precio de venta' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Para poder anunciar este artículo en el Mercado de la Comunidad deberás confirmarlo en tu aplicación Steam Mobile. Solo tienes que iniciar la aplicación e ir a la página Confirmaciones del menú.' +
							'<br><br>' +
							'Si no ves la opción Confirmaciones en el menú principal de la aplicación, asegúrate de que tienes su versión más reciente.';

				}
				else
				{
					strText = 'Para poder publicar este artículo en el Mercado de la Comunidad, primero debes completar un paso de verificación adicional. Se ha enviado un email a tu dirección terminada en "%s" con más instrucciones.'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'Ha habido un problema al anunciar el artículo. Actualiza la página e inténtalo de nuevo.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Ha habido un problema al guardar tu selección. Por favor, inténtalo de nuevo más tarde.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Precios medios de venta', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d vendido(s)'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Usa esto para denunciar a %s ante el Soporte de Steam por un intento de estafa.<br><br>Por favor, proporciona una breve descripción:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Denunciar estafa', $Dialog, 'Denunciar estafa').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Denunciar estafa', 'Es necesaria una descripción.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( '¡Gracias!', 'Tu denuncia ha sido enviada y será revisada por el Soporte de Steam.' );
		}).fail( function() {
			ShowAlertDialog( 'Denunciar estafa', 'Ha habido un problema al guardar tu denuncia. Por favor, inténtalo de nuevo más tarde.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">¿Qué es una retención en el intercambio o el Mercado?</div>' );
	description.append( '<div class="escrow_explanation_a">Las retenciones son periodos de tiempo durante los que Steam retiene los artículos mientras se completa un intercambio o se publica un artículo en el Mercado.</div>' );

	description.append( '<div class="escrow_explanation_q">Las retenciones ayudan a proteger tus artículos</div>' );
	description.append( '<div class="escrow_explanation_a">Las cuentas de Steam son valiosas, especialmente si tienen artículos que merece la pena robar. Por eso, si no has protegido tu cuenta con un dispositivo físico (el Autenticador Móvil de Steam Guard), la retención de un intercambio te dará tiempo a descubrir que la seguridad de tu cuenta se ha visto comprometida y a evitar que los artículos salgan de tu inventario.</div>' );

	description.append( '<div class="escrow_explanation_q">Un retraso para impedir el robo de artículos</div>' );
	description.append( '<div class="escrow_explanation_a">Si un usuario que intercambia o vende artículos no ha tenido protegida su cuenta con el autenticador móvil durante los últimos 7 días: <br><span style="color: #5aa9d6">Intercambios:</span> Steam retrasará la entrega de artículos de intercambios completados hasta 15 días.<br><span style="color: #5aa9d6">Mercado:</span> Steam retrasará la publicación de los anuncios de venta hasta 15 días.<br><br>Esto proporciona al usuario un plazo para cancelar transacciones pendientes no autorizadas.<br><br>La cancelación de intercambios pendientes o retenidos iniciará un periodo de espera en tu cuenta para evitar cualquier intento no autorizado de intercambio de artículos. La cancelación de anuncios del Mercado no conlleva tiempos de espera.</div>' );

	description.append( '<div class="escrow_explanation_q">Elimina la necesidad de retener los artículos</div>' );
	description.append( '<div class="escrow_explanation_a">Usar un autenticador móvil garantiza que solo tú puedas intercambiar y vender tus artículos de forma rápida y segura, de modo que las retenciones ya no sean necesarias. Aumenta la seguridad de tu cuenta con el <a href="https://store.steampowered.com/mobile/" target="_blank">Autenticador Móvil de Steam Guard</a> para dispositivos iOS, Android y Windows Phone. </div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Lee la entrada del blog de Steam sobre seguridad de cuentas</a> para una explicación detallada sobre por qué aplicamos las retenciones.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Retenciones en el intercambio y el Mercado',
			description,
			'Más información acerca del Autenticador Móvil de Steam Guard',
			'Cerrar'
		);
