...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Ошибка подтверждения',
					'Произошла ошибка при выполнении этого действия. Пожалуйста, попробуйте позже.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Ошибка подтверждения',
					'Во время отмены этого предложения произошла ошибка. Пожалуйста, попробуйте позже.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Ошибка подтверждения',
							typeof responseData.message != 'undefined' ? responseData.message : 'При загрузке подробностей для этого подтверждения возникла проблема. Пожалуйста, попробуйте позже.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Ошибка подтверждения',
						'При загрузке подробностей для этого подтверждения возникла проблема. Пожалуйста, попробуйте позже.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Ошибка подтверждения',
				'Невозможно загрузить информацию об этом подтверждении. Пожалуйста, перезагрузите устройство и попробуйте снова. Если проблему не удалось устранить, вероятно, Steam Guard был установлен на другом устройстве, и теперь коды генерируются на нём. Вы не можете использовать аутентификатор, установленный на этом устройстве.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Отменить отмеченное' );
			$btnAccept.text( 'Подтвердить отмеченное' );
