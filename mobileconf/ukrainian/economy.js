...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Показати ще' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Невідомий предмет ' + itemid, descriptions: [], fraudwarnings: [ 'Не вдалося отримати інформацію про цей предмет.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Вилучити з обміну' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Додати до обміну' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Усі предмети з #{appname}');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Інвентар не доступний для перегляду. Будь ласка, спробуйте ще раз пізніше. ' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"\u0412\u0438 \u043c\u043e\u0436\u0435\u0442\u0435 \u0457\u0445 \u043e\u0442\u0440\u0438\u043c\u0430\u0442\u0438 \u0437 \u0432\u0438\u043f\u0430\u0434\u0456\u043d\u044c \u043f\u0440\u0435\u0434\u043c\u0435\u0442\u0456\u0432 \u043f\u0456\u0434 \u0447\u0430\u0441 \u0433\u0440\u0438, \u043f\u0440\u0438\u0434\u0431\u0430\u0442\u0438 \u0443 \u043a\u0440\u0430\u043c\u043d\u0438\u0446\u0456 \u041c\u0430\u043d\u043d \u041a\u043e \u0430\u0431\u043e \u0432\u0438\u043c\u0456\u043d\u044f\u0442\u0438 \u0443 \u0434\u0440\u0443\u0437\u0456\u0432."},"620":{"howtoget":"\u0412\u0438 \u043c\u043e\u0436\u0435\u0442\u0435 \u043f\u0440\u0438\u0434\u0431\u0430\u0442\u0438 \u0457\u0445 \u0432 \u0456\u0433\u0440\u043e\u0432\u0456\u0439 \u043a\u0440\u0430\u043c\u043d\u0438\u0446\u0456 Portal 2, \u0430\u0431\u043e \u0432\u0438\u043c\u0456\u043d\u044f\u0442\u0438 \u0443 \u0432\u0430\u0448\u0438\u0445 \u0434\u0440\u0443\u0437\u0456\u0432."},"753":{"1":{"howtoget":"\u0412\u0438 \u043c\u043e\u0436\u0435\u0442\u0435 \u043e\u0442\u0440\u0438\u043c\u0430\u0442\u0438 \u0434\u043e\u0434\u0430\u0442\u043a\u043e\u0432\u0456 \u043a\u043e\u043f\u0456\u0457 \u0456\u0433\u043e\u0440 \u0443 \u0441\u043a\u043b\u0430\u0434\u0456 \u043e\u0441\u043e\u0431\u043b\u0438\u0432\u0438\u0445 \u043f\u0440\u043e\u043f\u043e\u0437\u0438\u0446\u0456\u0439. \u0410\u0431\u043e \u0436 \u043f\u0440\u0438\u0434\u0431\u0430\u0439\u0442\u0435 \u0433\u0440\u0443 \u0432 \u043a\u0440\u0430\u043c\u043d\u0438\u0446\u0456 Steam, \u043e\u0431\u0440\u0430\u0432\u0448\u0438 \u00ab\u041f\u0440\u0438\u0434\u0431\u0430\u0442\u0438 \u044f\u043a \u0434\u0430\u0440\u0443\u043d\u043e\u043a\u00bb \u043f\u0440\u0438 \u043e\u043f\u043b\u0430\u0442\u0456."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> з #{contextname}');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Ця кількість є приватною і показана лише вам.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s самоцвітів'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Переглянути на ринку спільноти' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Опрацювання…">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Починаючи з: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Немає доступних пропозицій для цього предмета.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = 'Продано за останні 24 години: %1$s';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Середня ціна: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Кількість: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Продати' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Ви не можете продавати предмети на Ринку, доки не <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">додасте кошти до свого гаманця Steam</a>, або не здійсните придбання у крамниці Steam та надасте вашу розрахункову адресу.',
			'Неможливо продати предмет'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Виставіть предмет на продаж' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Ви маєте прийняти умови Угоди підписника Steam, щоби продати цей предмет.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Вам потрібно ввести дійсну кількість.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Введена ціна перевищує максимально допустиму, яка дорівнює %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Ви повинні ввести коректну ціну.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Підтвердження ціни' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Для продажу цього предмета у крамниці спільноти ви повинні підтвердити пропозицію у своєму мобільному застосунку Steam. Запустіть мобільний застосунок та за допомогою меню перейдіть на сторінку підтвердження.' +
							'<br><br>' +
							'Якщо у головному меню немає опції підтвердження, упевніться, що у вас найновіша версія застосунку.';

				}
				else
				{
					strText = 'Для продажу цього предмета на ринку спільноти ви маєте здійснити ще одну перевірочну дію. На вашу електронну адресу (закінчується на «%s») надіслано листа з подальшими вказівками.'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'Виникла проблема під час виставлення вашого предмета на продаж. Оновіть сторінку та спробуйте знову.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Виникла проблема під час збереження вашого вибору. Будь ласка, спробуйте знову пізніше.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Медіана вартості', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>Продано %d шт.'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Відправте скаргу на %s до служби підтримки Steam, якщо вас намагалися обдурити.<br><br>Будь ласка, надайте короткий опис:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Повідомити про шахрайство', $Dialog, 'Повідомити про шахрайство').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Повідомити про шахрайство', 'Потрібен опис.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Дякуємо!', 'Ваша скарга була надіслана ​​та буде розглянута службою підтримки Steam.' );
		}).fail( function() {
			ShowAlertDialog( 'Повідомити про шахрайство', 'Виникла проблема під час збереження вашої скарги. Будь ласка, спробуйте знову пізніше.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Що таке утримування обміну чи торгівлі?</div>' );
	description.append( '<div class="escrow_explanation_a">Утримування — це період часу, протягом якого предмети утримуються в Steam до завершення обміну або виставлення пропозиції на ринку.</div>' );

	description.append( '<div class="escrow_explanation_q">Утримування предметів дає змогу убезпечити ваші предмети від крадіжки</div>' );
	description.append( '<div class="escrow_explanation_a">Акаунти Steam вартісні, особливо, якщо вони мають достатньо цінні для викрадення предмети. Якщо ви не захистили свій акаунт за допомогою фізичного пристрою (мобільний автентифікатор Steam Guard), утримування обмінів надасть вам вдосталь часу для розуміння, що ваш акаунт зламаний, а також, ви матимете змогу перешкоджати переміщенню предметів зі свого акаунта.</div>' );

	description.append( '<div class="escrow_explanation_q">Затримка для виявлення і запобігання крадіжки предметів</div>' );
	description.append( '<div class="escrow_explanation_a">Якщо акаунт користувача, який обмінюється предметами чи продає їх, не перебував під захистом мобільного автентифікатора протягом останніх 7 днів: <br><span style="color: #5aa9d6">Обміни:</span> Steam затримуватиме доставку предметів завершеного обміну максимум на 15 дн. <br><span style="color: #5aa9d6">Продаж:</span> Steam затримуватиме виставлення пропозицій перед їхньою публікацією максимум на 15 дн. <br><br>Це надасть користувачу час для скасування будь-яких незавершених трансакцій, які він не авторизовував.<br><br>Після скасування пропозицій обміну, що очікуються чи утримуються, обмін на вашому акаунті буде тимчасово недоступний задля запобігання подальших несанкціонованих спроб виміняти предмети. Скасування виставленої пропозиції на ринку спільноти не передбачає жодних тимчасових обмежень.</div>' );

	description.append( '<div class="escrow_explanation_q">Усунення необхідності утримування предметів</div>' );
	description.append( '<div class="escrow_explanation_a">Використання мобільного автентифікатора гарантує те, що ви і тільки ви зможете швидко і безпечно обмінювати предмети без необхідності мати утримування обмінів. Збільшіть безпеку свого акаунта, отримавши <a href="https://store.steampowered.com/mobile/" target="_blank">мобільний автентифікатор Steam Guard</a> для пристроїв на iOS та Android. </div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Перегляньте статтю щодо убезпечення акаунтів у блозі Steam</a>, щоби зрозуміти навіщо ми ввели утримування предметів.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Утримування обмінів та продажів',
			description,
			'Більше відомостей про мобільний автентифікатор Steam Guard',
			'Закрити'
		);
