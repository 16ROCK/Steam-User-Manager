...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Помилка підтвердження',
					'Виникла проблема під час виконання цієї дії. Будь ласка, спробуйте свій запит знову пізніше.',
					'Гаразд'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Помилка підтвердження',
					'Виникла проблема під час скасування цього підтвердження. Будь ласка, спробуйте свій запит знову пізніше.',
					'Гаразд'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Помилка підтвердження',
							typeof responseData.message != 'undefined' ? responseData.message : 'Виникла проблема під час завантаження відомостей для цього підтвердження. Будь ласка, спробуйте свій запит знову пізніше.',
							'Гаразд'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Помилка підтвердження',
						'Виникла проблема під час завантаження відомостей для цього підтвердження. Будь ласка, спробуйте свій запит знову пізніше.',
						'Гаразд'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Помилка підтвердження',
				'Мобільний застосунок не може завантажити відомості для цього підтвердження. Будь ласка, перезапустіть свій пристрій та повторіть запит пізніше. Якщо проблема не зникає, то можливо, інший пристрій було налаштовано для постачання кодів Steam Guard для вашого акаунта, тобто автентифікатор на поточному пристрої більше не є дійсним.',
				'Гаразд'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Скасувати вибране' );
			$btnAccept.text( 'Підтвердити вибране' );
