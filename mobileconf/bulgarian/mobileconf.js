...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Грешка при потвърждение',
					'Имаше проблем при изпълняването на това действие. Моля, опитайте заявката си отново по-късно.',
					'Добре'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Грешка при потвърждение',
					'Имаше проблем при отказа на това потвърждение. Моля, опитайте заявката си отново по-късно.',
					'Добре'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Грешка при потвърждение',
							typeof responseData.message != 'undefined' ? responseData.message : 'Имаше проблем при зареждането на подробностите за това потвърждение. Моля, опитайте заявката си отново по-късно.',
							'Добре'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Грешка при потвърждение',
						'Имаше проблем при зареждането на подробностите за това потвърждение. Моля, опитайте заявката си отново по-късно.',
						'Добре'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Грешка при потвърждение',
				'Мобилното приложение е неспособно да зареди подробности за това потвърждение. Моля, рестартирайте своето устройство и опитайте заявката отново по-късно. Ако все още се натъквате на проблем, вероятно някое друго устройство е зададено да предоставя Steam Guard кодове за акаунта Ви. Това означава, че удостоверителя на това устройство вече е невалиден.',
				'Добре'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Отказване на избраното' );
			$btnAccept.text( 'Потвърждаване на избраното' );
