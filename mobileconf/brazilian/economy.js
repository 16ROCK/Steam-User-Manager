...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Exibir mais' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Item desconhecido ' + itemid, descriptions: [], fraudwarnings: [ 'Não foi possível adquirir informações sobre este item.' ] };
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Todos os itens de #{appname}');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Este inventário não está disponível no momento. Tente novamente mais tarde.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Voc\u00ea pode obt\u00ea-los gratuitamente ao jogar, compr\u00e1-los na Loja Mann Co. ou troc\u00e1-los com amigos."},"620":{"howtoget":"Voc\u00ea pode obt\u00ea-los da loja do Portal 2 dentro do jogo ou troc\u00e1-los com amigos."},"753":{"1":{"howtoget":"Voc\u00ea pode obter c\u00f3pias extras de jogos durante promo\u00e7\u00f5es especiais ou ao comprar um jogo da Loja Steam e selecionar \"Comprar como presente\" durante a finaliza\u00e7\u00e3o do pedido."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Esta quantidade é privada e somente você pode vê-la.' );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Processando...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Não há anúncios para este item.' + '<br>';
						}
						...
						if ( transport.responseJSON.volume )
						{
							...
							//strInfo += 'Preço médio: ' + transport.responseJSON.median_price + '<br>';
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Anunciar' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Você não pode vender itens no Mercado da Comunidade até <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">adicionar fundos à sua Carteira Steam</a> ou efetuar alguma compra na loja Steam e fornecer o seu endereço de cobrança.',
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Coloque um item à venda' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Você precisa aceitar os termos do Acordo de Assinatura do Steam para vender este item.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Insira uma quantidade válida.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' O preço inserido excede o preço máximo de %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'É necessário inserir um preço válido.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Confirme o preço de venda' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Para anunciar este item no Mercado da Comunidade, abra o aplicativo móvel do Steam no seu dispositivo, acesse a página Confirmações no menu e confirme-o.' +
							'<br><br>' +
							'Caso não veja a opção de confirmações no menu principal do aplicativo, confirme que está usando a versão mais recente do mesmo.';

				}
				else
				{
					strText = 'Para anunciar este item no Mercado da Comunidade, você precisa realizar uma etapa adicional de verificação. Um e-mail foi enviado ao seu endereço (de final "%s") com instruções adicionais.'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'Ocorreu um problema ao anunciar o seu item. Atualize a página e tente novamente.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Houve um problema ao salvar a sua seleção. Tente novamente mais tarde.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Valores medianos de venda', textAlign: 'left' },
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Use este formulário para denunciar %s ao Suporte Steam por uma tentativa de golpe.<br><br>Forneça uma breve descrição:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Denunciar golpe', $Dialog, 'Denunciar golpe').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Denunciar golpe', 'Uma descrição é necessária.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Obrigado!', 'A sua denúncia foi enviada e será analisada pelo Suporte Steam.' );
		}).fail( function() {
			ShowAlertDialog( 'Denunciar golpe', 'Houve um problema ao salvar a sua denúncia. Tente novamente mais tarde.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">O que são retenções de troca e Mercado?</div>' );
	description.append( '<div class="escrow_explanation_a">Uma retenção é um período de tempo em que os itens são retidos pelo Steam antes da realização de uma troca ou anúncio de um item no Mercado.</div>' );

	description.append( '<div class="escrow_explanation_q">A retenção de itens ajuda a proteger os seus itens</div>' );
	description.append( '<div class="escrow_explanation_a">Contas Steam são valiosas, ainda mais se tiverem itens de alto valor. Caso não tenha protegido a sua conta com um dispositivo físico (o autenticador móvel do Steam Guard), a retenção dará tempo para que descubra que a sua conta foi invadida, impedindo que os seus itens deixem a sua conta.</div>' );

	description.append( '<div class="escrow_explanation_q">Um atraso para impedir o roubo de itens</div>' );
	description.append( '<div class="escrow_explanation_a">Caso um usuário que esteja dando itens não tenha protegido a conta com um autenticador móvel pelos últimos 7 dias: <br><span style="color: #5aa9d6">Trocas:</span>  a entrega dos itens será adiada pelo Steam em, no máximo, 15 dias <br><span style="color: #5aa9d6">Mercado:</span>  anúncios serão retidos pelo Steam antes de serem publicados por, no máximo, 15 dias <br><br> Isso permite que o usuário tenha tempo para cancelar transações não autorizadas. <br><br>Cancelar trocas pendentes ou retidas fará com que a sua conta seja proibida de trocar temporariamente para impedir outras trocas não autorizadas. Não há penalidades por cancelar anúncios do Mercado.</div>' );

	description.append( '<div class="escrow_explanation_q">Evite a retenção de itens</div>' );
	description.append( '<div class="escrow_explanation_a">Use um autenticador móvel para confirmar transações e assegurar que você — e só você — pode trocar ou vender os seus itens de forma rápida e segura. Aumente a segurança da sua conta com o <a href="https://store.steampowered.com/mobile/" target="_blank">autenticador móvel do Steam Guard</a> para dispositivos iOS, Android e Windows Phone.</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Leia o artigo sobre segurança de conta no blog do Steam</a> para saber por que implementamos retenções.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Retenções de troca e Mercado',
			description,
			'Saiba mais sobre o autenticador móvel do Steam Guard',
			'Fechar'
		);
