...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Erro de confirmação',
					'Houve um problema ao realizar a ação. Tente realizar a solicitação novamente mais tarde.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Erro de confirmação',
					'Houve um problema ao cancelar a confirmação. Tente realizar a solicitação novamente mais tarde.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Erro de confirmação',
							typeof responseData.message != 'undefined' ? responseData.message : 'Houve um problema ao carregar os detalhes da confirmação. Tente realizar a solicitação novamente mais tarde.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Erro de confirmação',
						'Houve um problema ao carregar os detalhes da confirmação. Tente realizar a solicitação novamente mais tarde.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Erro de confirmação',
				'Não foi possível carregar os detalhes da confirmação no aplicativo móvel. Reinicie o dispositivo e tente realizar a confirmação mais tarde. Caso o problema persista, pode ser que outro dispositivo tenha sido configurado para fornecer os códigos do Steam Guard para a conta em questão, invalidando o autenticador no dispositivo atual.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Cancelar selecionados' );
			$btnAccept.text( 'Confirmar selecionados' );
