...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Megerősítési hiba',
					'Probléma adódott a művelet végrehajtásakor. Próbáld újra a kérést később.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Megerősítési hiba',
					'Probléma adódott a megerősítés megszakításakor. Próbáld újra a kérést később.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Megerősítési hiba',
							typeof responseData.message != 'undefined' ? responseData.message : 'Probléma adódott a megerősítéshez tartozó részletek betöltésekor. Próbáld újra a kérést később.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Megerősítési hiba',
						'Probléma adódott a megerősítéshez tartozó részletek betöltésekor. Próbáld újra a kérést később.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Megerősítési hiba',
				'A mobilalkalmazás nem tudja betölteni a megerősítés részleteit. Indítsd újra az eszközöd és próbáld újra később. Ha még mindig problémába ütközöl, előfordulhat, hogy egy másik eszköz lett beállítva Steam Guard kódok generálására a fiókodhoz, ami azt jelenti, hogy az ezen az eszközön lévő hitelesítő már nem érvényes.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Kijelöltek törlése' );
			$btnAccept.text( 'Kijelöltek megerősítése' );
