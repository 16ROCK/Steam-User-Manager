...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Továbbiak mutatása' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Ismeretlen tárgy ' + itemid, descriptions: [], fraudwarnings: [ 'Nem sikerült információt kapni erről a tárgyról.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Kivétel a cseréből' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Cseréhez adás' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Minden #{appname} tárgy');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Ez a raktár jelenleg nem elérhető. Próbáld meg újra később.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Beszerezheted \u0151ket ingyen, a j\u00e1t\u00e9kban esett t\u00e1rgyakk\u00e9nt, a j\u00e1t\u00e9kon bel\u00fcli Mann. Co \u00c1ruh\u00e1zban, vagy cser\u00e9lhetsz bar\u00e1tokkal."},"620":{"howtoget":"Beszerezheted \u0151ket a Portal 2 j\u00e1t\u00e9kon bel\u00fcli boltj\u00e1ban, vagy cser\u00e9lhetsz bar\u00e1tokkal."},"753":{"1":{"howtoget":"Plusz j\u00e1t\u00e9kp\u00e9ld\u00e1nyokat szerezhetsz k\u00fcl\u00f6nleges aj\u00e1nlatok idej\u00e9n, vagy ha az \u201eAj\u00e1nd\u00e9kba v\u00e1s\u00e1rolom\u201d lehet\u0151s\u00e9get v\u00e1lasztod a Steam \u00c1ruh\u00e1zb\u00f3l val\u00f3 v\u00e1s\u00e1rl\u00e1skor."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '#{contextname} - <span style="#{currencystyle}">#{amount}</span>');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Ez a mennyiség privát, és csak te látod.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s drágakő'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Megnézés a Közösségi Piacon' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Feldolgozás...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Kezdő ár: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Jelenleg nincsenek elérhető ajánlatok ehhez a tételhez.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '%1$s darab az elmúlt 24 órában';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Ár középértéke: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Eladva: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Eladás' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Nem adhatsz el a közösségi piacon, amíg <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">elég összeget nem teszel Steam Pénztárcádba</a>, vagy nem vásárolsz a Steam boltban és meg nem adod számlázási címedet.',
			'Tétel nem adható el'
		);
	}
	else
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Tétel felajánlása értékesítésre' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'A tétel értékesítéséhez el kell fogadnod a Steam előfizetői szerződés feltételeit.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Érvényes mennyiséget kell megadnod.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' A megadott ár meghaladja a maximális %1$s árat.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Érvényes árat adj meg.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Értékesítési ár jóváhagyása' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Ahhoz, hogy felkínálhasd ezt a tárgyat a Közösségi Piacon, ellenőrizned kell azt a Steam mobilalkalmazásban. Ezt megteheted az alkalmazás elindításával, és a menüből a Megerősítések oldalra navigálással.' +
							'<br><br>' +
							'Ha nem látod a „Megerősítés” lehetőséget az alkalmazás főmenüjében, győződj meg róla, hogy az alkalmazás legújabb verziójával rendelkezel.';

				}
				else
				{
					strText = 'E tárgy felajánlásához a Közösségi Piacon teljesítened kell egy további ellenőrzési lépést. Egy e-mailt küldtünk a címedre („%s” a végződése) a további utasításokkal.'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'Probléma adódott tételed felajánlásakor. Frissítsd az oldalt, és próbáld újra.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Probléma adódott a választásod mentésekor, próbáld újra később.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Eladási árak középértéke', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d eladott'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Ezzel jelentheted %s felhasználót a Steam Támogatásnak átverés kísérlete miatt.<br><br>Kérjük, adj rövid leírást:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Átverés jelentése', $Dialog, 'Átverés jelentése').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Átverés jelentése', 'Leírás szükséges.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Köszönjük!', 'A jelentésed továbbításra került, és a Steam Támogatás át fogja nézni.' );
		}).fail( function() {
			ShowAlertDialog( 'Átverés jelentése', 'Probléma adódott jelentésed mentése közben. Próbáld újra később.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Mi az a csere- vagy piaci várakoztatás?</div>' );
	description.append( '<div class="escrow_explanation_a">A várakoztatás olyan időszak, mely során a Steam visszatartja a tételeket, mielőtt egy csere lezajlana, vagy egy piaci ajánlat közzétételre kerülne.</div>' );

	description.append( '<div class="escrow_explanation_q">A tételvárakoztatás segít megvédeni a tárgyaidat</div>' );
	description.append( '<div class="escrow_explanation_a">A Steam fiókok értékesek, főleg, ha ellopásra érdemes tárgyak vannak rajtuk. Ha nem véded fiókodat fizikai eszközzel (a Steam Guard mobilhitelesítő használatával), a cserevárakoztatás időt fog adni, hogy felfedezd fiókod biztonságának sérülését és megakadályozd, hogy tárgyak hagyják el a fiókodat.</div>' );

	description.append( '<div class="escrow_explanation_q">Késleltetés a tárgylopások elcsípésére és megakadályozására</div>' );
	description.append( '<div class="escrow_explanation_a">Ha egy tárgyakat elcserélő vagy eladó felhasználó nem védte fiókját legalább 7 napja mobilhitelesítővel: <br><span style="color: #5aa9d6">Cserék:</span> a Steam a lezárt cserék utáni tárgyleszállítást legfeljebb 15 napig késlelteti.<br><span style="color: #5aa9d6">Piac:</span> a Steam közzétételük előtt legfeljebb 15 napig visszatartja az eladási ajánlatokat.<br><br>Ez időt ad a felhasználónak bármely olyan függő tranzakció engedélyezésére, amit nem hagyott jóvá. <br><br>Függő vagy várakoztatásban levő cserék visszavonása cserevárakoztatást indít a fiókodon bármely további engedélyezetlen tárgycsere megkísérlésének megelőzésére. Piaci ajánlatok visszavonása után nincs várakoztatás.</div>' );

	description.append( '<div class="escrow_explanation_q">Oldd fel a tételvisszatartás szükségességét</div>' );
	description.append( '<div class="escrow_explanation_a">A mobilhitelesítő használata biztosítja, hogy csak te tudd gyorsan és biztonságosan elcserélni vagy eladni tárgyaidat, így a továbbiakban nem szükséges a várakoztatás. Növeld fiókod biztonságát a <a href="https://store.steampowered.com/mobile/" target="_blank">Steam Guard mobilhitelesítő</a> beszerzésével iOS és Android eszközökre.</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Olvasd el a Steam blogbejegyzést a fiókbiztonságról</a> részletes magyarázatért arról, miért vezettük be a várakoztatásokat.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Csere- és piaci várakoztatások',
			description,
			'Tudj meg többet a Steam Guard mobilhitelesítőről',
			'Bezárás'
		);
