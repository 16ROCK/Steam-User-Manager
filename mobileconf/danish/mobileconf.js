...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Bekræftelsesfejl',
					'Der opstod et problem under udførelsen af handlingen. Prøv venligst din anmodning igen senere.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Bekræftelsesfejl',
					'Der opstod et problem under annulleringen af bekræftelsen. Prøv venligst din anmodning igen senere.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Bekræftelsesfejl',
							typeof responseData.message != 'undefined' ? responseData.message : 'Der opstod et problem under indlæsningen af bekræftelsesoplysningerne. Prøv venligst din anmodning igen senere.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Bekræftelsesfejl',
						'Der opstod et problem under indlæsningen af bekræftelsesoplysningerne. Prøv venligst din anmodning igen senere.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Bekræftelsesfejl',
				'Mobil-appen kan ikke indlæse detaljerne om bekræftelsen. Genstart din enhed og prøv din anmodning igen senere. Hvis du stadig oplever problemer, kan det være, at en anden enhed er blevet oprettet til at sende Steam Guard-koder til din konto, hvilket betyder, at enhedens authenticator ikke længere er gyldig.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Annuller valgte' );
			$btnAccept.text( 'Bekræft valgte' );
