...
var CInventory = Class.create( {
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Ukendt genstand ' + itemid, descriptions: [], fraudwarnings: [ 'Kunne ikke hente informationer om denne genstand.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Fjern fra byttehandel' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Føj til byttehandel' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Alle #{appname}-genstande');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Dette inventar er ikke tilgængeligt på nuværende tidspunkt. Prøv venligst igen senere.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Du kan f\u00e5 dem gratis igennem spillets genstandsdrop-system, i spillets Mann Co.-butik, eller ved at bytte dig til dem hos venner."},"620":{"howtoget":"Du kan f\u00e5 dem i butikken i Portal 2 eller bytte dig til dem hos venner."},"753":{"1":{"howtoget":"Du kan f\u00e5 ekstra eksemplarer af spil ved specielle tilbud eller ved at k\u00f8be et spil fra Steam-butikken og v\u00e6lge \u201dK\u00f8b som gave\u201d ved betalingen."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> fra #{contextname}');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Dette beløb er privat og kan kun ses af dig.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s ædelsten'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Vis på fællesskabsmarkedet' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Arbejder...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Starter fra: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Der er på nuværende tidspunkt ikke nogen opslag tilgængelig for denne genstand.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '%1$s solgt i de seneste 24 timer';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Medianpris: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Antal: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Sælg' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Du kan ikke sælge genstande på fællesskabesmarkedet, før du <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">sætter penge ind på din Steam-tegnebog</a> eller foretager et køb i Steam-butikken og oplyser din faktureringsadresse.',
			'Kan ikke sælge genstand'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Sæt en genstand til salg' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Du skal acceptere betingelserne i Steam-abonnentaftalen for at sælge denne genstand.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Du skal indtaste et gyldigt antal.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Den angivne pris overstiger den maksimale pris på %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Du skal indtaste en gyldig pris.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Bekræft salgspris' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'For at slå denne genstand op på fællesskabsmarkedet skal du bekræfte opslaget i din Steam-mobilapp. Du kan bekræfte det ved at starte app\'en og navigere til siden "Bekræftelser" fra menuen.' +
							'<br><br>' +
							'Hvis du ikke ser menupunktet "Bekræftelser" i app\'ens hovedmenu, så sørg for at du har den seneste version af app\'en.';

				}
				else
				{
					strText = 'For at slå denne genstand op på fællesskabsmarkedet skal du gennemføre et yderligere bekræftelsestrin. Der er blevet sendt en e-mail til din e-mailadresse (som slutter på "%s") med yderligere instruktioner.'.replace( /%s/, transport.responseJSON.email_domain );
				}

				ShowAlertDialog(
						'Yderligere bekræftelse påkrævet',
						strText
				);
			}
			else
		...
		{
			this.DisplayError( 'Der opstod et problem med opstillingen af din genstand. Opdater siden og prøv igen.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Der opstod et problem i forsøget på at gemme dit valg, prøv venligst igen senere.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Mediansalgspriser', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d solgt'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Brug dette til at rapportere %s til Steam Support for forsøg på svindel.<br><br>Giv venligst en kort beskrivelse:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Anmeld svindel', $Dialog, 'Anmeld svindel').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Anmeld svindel', 'En beskrivelse er påkrævet.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Tak!', 'Din rapport er blevet indsendt og vil blive gennemgået af Steam Support.' );
		}).fail( function() {
			ShowAlertDialog( 'Anmeld svindel', 'Der opstod et problem i forsøget på at gemme din rapportering. Prøv venligst igen senere.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Hvad er en bytte- eller markedsforsinkelse?</div>' );
	description.append( '<div class="escrow_explanation_a">En forsinkelse er en tidsperiode, hvor genstande forsinkes af Steam, før en byttehandel gennemføres eller et markedsopslag slås op.</div>' );

	description.append( '<div class="escrow_explanation_q">Genstandsforsinkelser hjælper med at beskytte dine genstande</div>' );
	description.append( '<div class="escrow_explanation_a">Steam-konti er kostbare; især hvis de har genstande, der er værd at stjæle. Hvis du ikke har beskyttet din konto med en fysisk enhed (Steam Guard-mobilauthenticatoren), så giver en bytteforsinkelse dig tid til at opdage, hvis din konto er blevet brudt, og det forhindrer, at dine genstande forlader din konto.</div>' );

	description.append( '<div class="escrow_explanation_q">En forsinkelse til at forhindre og standse tyveri af genstande</div>' );
	description.append( '<div class="escrow_explanation_a">Hvis en bruger, der bytter eller sælger genstande, ikke har haft sin konto beskyttet af en mobilauthenticator de seneste 7 dage: <br><span style="color: #5aa9d6">Byttehandler:</span> forsinkes levering af genstande, fra gennemførte byttehandler, af Steam i op til 15 dage  <br><span style="color: #5aa9d6">Markedet:</span> forsinkes salgsopslag af Steam, før de slås op, i op til 15 dage <br><br>Dette giver brugeren tid til at annullere enhver transaktion, vedkommende ikke har godkendt.<br><br>At annullere afventende eller forsinkede byttehandler indleder en byttenedkøling på din konto for at forhindre yderligere uautoriserede forsøg på at bytte genstande væk. Der er ingen nedkøling ved annullering af markedsopslag.</div>' );

	description.append( '<div class="escrow_explanation_q">Fjern behovet for genstandsforsinkelser</div>' );
	description.append( '<div class="escrow_explanation_a">Ved at bruge en mobilauthenticator sikrer du, at du, og kun du, kan bytte eller sælge dine genstande hurtigt og sikkert, hvilket medfører, at forsinkelser ikke længere er nødvendige. Øg din kontosikkerhed ved at hente <a href="https://store.steampowered.com/mobile/" target="_blank">Steam Guard-mobilauthenticatoren</a> til iOS- og Android-enheder.</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Læs Steam-blogindlægget om kontosikkerhed</a> for at få en detaljeret forklaring på hvorfor vi implementerer forsinkelser.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Bytte- og markedsforsinkelser',
			description,
			'Lær om Steam Guard-mobilauthenticatoren',
			'Luk'
		);
