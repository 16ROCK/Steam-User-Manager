...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'確認錯誤',
							typeof responseData.message != 'undefined' ? responseData.message : '載入該確認資訊時遇到問題。請稍後再重試您的請求。',
							'確定'
					).always( function() {
						window.history.back();
					} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'確認錯誤',
				'行動應用程式無法載入該確認的詳細資訊，請重新啟動您的裝置，並稍後再重試您的請求。如果仍遇到問題，則很可能是因為您在另一個不同的裝置上設定了 Steam Guard 行動驗證器，使得此裝置上的驗證器失效。',
				'確定'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( '取消所選項目' );
			$btnAccept.text( '確認所選項目' );
