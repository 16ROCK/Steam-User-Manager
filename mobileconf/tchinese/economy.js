...
var CInventory = Class.create( {
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: '未知物品 ' + itemid, descriptions: [], fraudwarnings: [ '無法取得關於此物品的資訊。' ] };
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( '所有 #{appname} 物品');
...
var g_rgEconomyDisplay = {"440":{"howtoget":"\u60a8\u53ef\u4ee5\u5728\u300a\u7d55\u5730\u8981\u585e 2\u300b\u904a\u6232\u5167\u7684\u7269\u54c1\u96a8\u6a5f\u6389\u843d\u4ee5\u53ca Mann Co. \u5546\u5e97\u6216\u662f\u8207\u597d\u53cb\u4ea4\u6613\u7372\u5f97\u5b83\u5011\u3002"},"620":{"howtoget":"\u60a8\u53ef\u4ee5\u5728\u300a\u50b3\u9001\u9580 2\u300b\u904a\u6232\u5167\u7684\u5546\u5e97\u6216\u662f\u8207\u597d\u53cb\u4ea4\u6613\u7372\u5f97\u5b83\u5011\u3002"},"753":{"1":{"howtoget":"\u60a8\u53ef\u4ee5\u8d81\u8457\u7279\u60e0\u6d3b\u52d5\u6216\u662f\u5f9e Steam \u5546\u5e97\u8cfc\u8cb7\u4e00\u6b3e\u904a\u6232\u7136\u5f8c\u5728\u7d50\u5e33\u6642\u9078\u64c7\u300c\u8d08\u79ae\u300d\u4f86\u7372\u5f97\u984d\u5916\u7684\u904a\u6232\u8907\u672c\u3002"}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> 來自 #{contextname}');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( '本物品是非公開項目，只有您自己能看見。' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s 枚寶石'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( '前往社群市集查看行情' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="執行中...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += '此物品目前架上缺貨。' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '在最近 24 小時內賣出了 %1$s 個';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += '中位數價格： ' + transport.responseJSON.median_price + '<br>';
							strInfo += '數量： ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', '販賣' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'您必須先<a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">新增資金至 Steam 錢包</a>或在 Steam 商店購物並提供帳單地址，才能在社群市集中販賣物品。',
			'無法販賣此物品'
		);
	}
	else
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( '將物品上架銷售' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( '您必須同意 Steam 訂戶協議中的條款才能販賣此項物品。' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' 輸入的價格超過 %1$s 的價格上限。'
		...
		$('market_sell_dialog_title').update( '確認售價' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = '若要將此物品上架至社群市集，您須先在 Steam 行動應用程式內進行確認。請登入應用程式，前往主選單中選擇「確認」選項。' +
							'<br><br>' +
							'若您在應用程式的主選單中看不到「確認」選項，請確保您的應用程式是最新版本。';

				}
				else
				{
					strText = '您必須先完成一道額外的驗證手續，才能將此物品上架至社群市集。我們已將內含詳細指示的電子郵件發送至您的電子信箱（結尾為「%s」）。'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( '上架您的物品時發生錯誤。請重新整理頁面，然後再試一次。' );
		}
	},
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: '中位販售價格', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d 已賣出'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( '請用此向我們的客服檢舉 %s 企圖進行詐騙。<br><br>請簡述請事件經過：'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( '檢舉詐騙', $Dialog, '檢舉詐騙').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( '檢舉詐騙', '請簡述事件過程。' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( '感謝您！', '您的檢舉已經送出，且將交由 Steam 客服審查。' );
		}).fail( function() {
			ShowAlertDialog( '檢舉詐騙', '保存您的回報時發生錯誤，請稍後再試。' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">什麼是交易與市集託管？</div>' );
	description.append( '<div class="escrow_explanation_a">託管是指在交易完成後或市集物品上架前，讓 Steam 託管物品一段時間。</div>' );

	description.append( '<div class="escrow_explanation_q">物品託管能幫助保護您的珍貴物品</div>' );
	description.append( '<div class="escrow_explanation_a">Steam 帳戶極具價值，尤其是當帳戶內有竊賊覬覦的高價物品時。如果您尚未使用實體裝置（Steam Guard 行動驗證器）來保護帳戶，交易託管將會給您足夠的時間來發現帳戶內的任何異常活動，防止物品在您不知情的狀況下被交易出去。</div>' );

	description.append( '<div class="escrow_explanation_q">為及時察覺與阻止盜賣物品所預留的緩衝時間</div>' );
	description.append( '<div class="escrow_explanation_a">若某位欲交易或販售物品的使用者在帳戶上啟用行動驗證器未滿 7 天：<br><span style="color: #5aa9d6">交易：</span>物品在交易確認後將由 Steam 託管達 15 天才發送至物品庫<br><span style="color: #5aa9d6">市集：</span>物品將由 Steam 託管達 15 天才上架販售<br><br>這將給予使用者足夠的時間來取消任何未經授權的交易動作。<br><br>取消任何待確認或託管中的交易將使帳戶進入交易冷卻期，以避免有人再次試圖未經授權把物品交易出去。將物品自市集下架則不受影響。</div>' );

	description.append( '<div class="escrow_explanation_q">移除物品託管的必要性</div>' );
	description.append( '<div class="escrow_explanation_a">使用行動驗證器可讓您既安全又快速地交易或出售自己的物品，同時防止他人以不當手法存取您的物品庫，因此託管便不再被視為必要的一層防護了。我們建議 iOS 和 Android 裝置的持有者下載 <a href="https://store.steampowered.com/mobile/" target="_blank">Steam Guard 行動驗證器</a>，提升帳戶安全性。</div>' );

	description.append( '<div class="escrow_explanation_a">想知道我們為什麼要實施物品託管制度，請<a href="http://store.steampowered.com/news/20631/" target="_blank">前往 Steam 網誌閱讀關於帳戶安全的文章</a>。</div>' );
