...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Montrer plus' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Item inconnu ' + itemid, descriptions: [], fraudwarnings: [ 'Échec de la récupération d\'informations sur cet item.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Retirer de l\'échange' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Ajouter à l\'échange' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Tous les items (#{appname})');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Cet inventaire n\'est pas disponible pour le moment. Veuillez réessayer plus tard.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Vous pouvez les obtenir gratuitement en les trouvant pendant une partie, via le magasin Mann Co. int\u00e9gr\u00e9 au jeu, ou via un \u00e9change avec vos contacts."},"620":{"howtoget":"Vous pouvez les obtenir depuis le magasin int\u00e9gr\u00e9 au jeu Portal 2, ou bien via un \u00e9change avec vos contacts."},"753":{"1":{"howtoget":"Vous pouvez obtenir des copies suppl\u00e9mentaires de jeux \u00e0 l'occasion d'offres sp\u00e9ciales, ou bien en choisissant l'option \"Acheter pour Offrir\" lors de l'achat d'un jeu sur le magasin Steam."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> de #{contextname}');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Ce montant est privé et n\'est vu que par vous.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s gemmes'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Voir sur le marché de la communauté' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Traitement en cours...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Prix de départ : ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Aucune offre n\'est actuellement disponible pour cet item.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '%1$s vendu(s) dans les dernières 24 heures';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Prix médian : ' + transport.responseJSON.median_price + '<br>';
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Vendre' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Vous ne pouvez pas vendre d\'items sur le marché de la communauté sans <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">ajouter des fonds à votre portemonnaie Steam</a> ou effectuer un achat dans le magasin Steam et fournir votre adresse de facturation.',
			'Impossible de vendre l\'item'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Mettre un item en vente' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Vous devez accepter les termes de l\'Accord de souscription Steam pour vendre cet item.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Vous devez saisir une quantité valide.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Le prix indiqué dépasse le prix maximal fixé à %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Vous devez indiquer un prix valide.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Confirmer le prix de vente' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Pour lister cet item sur le marché de la communauté, vous devez vérifier l\'offre via l\'application mobile Steam. Vous pouvez la vérifier en lançant l\'application et en naviguant vers la page « Confirmations » à partir du menu.' +
							'<br><br>' +
							'Si l\'option « Confirmations » n\'apparaît pas dans le menu principal de l\'appli, assurez-vous que vous possédez la dernière version de l\'application.';

				}
				else
				{
					strText = 'Pour lister cet item sur le marché de la communauté, vous devez remplir une étape de vérification supplémentaire. Un e-mail a été envoyé à votre adresse (se terminant par « %s ») avec des instructions supplémentaires.'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'Un problème est survenu lors de la mise en ligne de l\'item. Actualisez la page puis réessayez.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Un problème a été rencontré lors de l\'enregistrement de votre sélection. Veuillez réessayer plus tard.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Prix de vente médians', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d vendu(s)'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Utilisez cette option pour signaler %s au Support Steam pour tentative d\'escroquerie.<br><br>Veuillez fournir une brève description :'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Signaler une arnaque', $Dialog, 'Signaler une arnaque').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Signaler une arnaque', 'Une description est requise.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Merci !', 'Votre signalement a été soumis et sera analysé par le Support Steam.' );
		}).fail( function() {
			ShowAlertDialog( 'Signaler une arnaque', 'Un problème est survenu lors de l\'enregistrement de votre signalement. Veuillez réessayer plus tard.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Qu\'est ce qu\'un délai d\'échange ou du marché ?</div>' );
	description.append( '<div class="escrow_explanation_a">Un délai d\'attente est une période de temps durant laquelle les items sont temporairement retenus par Steam avant que l\'échange ne soit complété ou l\'offre sur le marché publiée.</div>' );

	description.append( '<div class="escrow_explanation_q">Les délais d\'attente vous aident à protéger vos items</div>' );
	description.append( '<div class="escrow_explanation_a">Les comptes Steam sont précieux, surtout si des items y sont associés. Si vous n’avez pas protégé votre compte à l’aide d’un appareil mobile (en utilisant l’authentificateur Mobile Steam Guard), le délai d\'attente vous permettra, en cas de vol de compte, d\'empêcher vos items de disparaître de votre compte.</div>' );

	description.append( '<div class="escrow_explanation_q">Un délai pour contrer le vol d\'items</div>' );
	description.append( '<div class="escrow_explanation_a">Lors d\'un échange, si l\'un des participants n\'a pas protégé son compte à l\'aide de l\'authentificateur mobile Steam Guard depuis au moins 7 jours : <br><span style="color: #5aa9d6">Échanges :</span> la livraison des items sera retardée par Steam pendant plusieurs jours (jusqu\'à 15 jours). <br><span style="color: #5aa9d6">Marché :</span> les offres de vente seront retenues par Steam jusqu\'à leur publication (jusqu\'à 15 jours). <br><br>Grâce aux délais d\'attente, les utilisateurs auront le temps d\'annuler tout échange ou offre en attente sur le marché dans le cas d\'une utilisation non-autorisée du compte. <br><br>Annuler un échange en cours ou en attente entrainera une restriction d\'échange sur le compte pour empêcher d\'autres tentatives d\'échange d\'items. Aucune restriction n\'est appliquée pour l\'annulation des offres de vente sur le marché.</div>' );

	description.append( '<div class="escrow_explanation_q">Évitez les délais d\'attente</div>' );
	description.append( '<div class="escrow_explanation_a">Utiliser un authentificateur mobile vous assure que vous, et seulement vous, pourrez échanger ou vendre des items rapidement et en toute sécurité. Les délais d\'attente ne seront ainsi plus nécessaires. Augmentez la sécurité de votre compte en activant l\'<a href="https://store.steampowered.com/mobile/" target="_blank">authentificateur mobile Steam Guard</a> pour iOS, Android et Windows Phone.</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Consultez la publication sur le blog Steam concernant la sécurité des comptes</a> pour une explication détaillée sur l\'ajout de délais d\'attente.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Délais d\'échange et du marché',
			description,
			'En savoir plus sur l\'authentificateur mobile Steam Guard',
			'Fermer'
		);
