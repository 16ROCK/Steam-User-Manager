...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Erreur de confirmation',
					'Un problème est survenu lors de l\'exécution de cette action. Veuillez réitérer votre demande ultérieurement.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Erreur de confirmation',
					'Un problème est survenu lors de l\'annulation de cette confirmation. Veuillez réitérer votre demande ultérieurement.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Erreur de confirmation',
							typeof responseData.message != 'undefined' ? responseData.message : 'Un problème est survenu lors du chargement des informations de cette confirmation. Veuillez réitérer votre demande ultérieurement.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Erreur de confirmation',
						'Un problème est survenu lors du chargement des informations de cette confirmation. Veuillez réitérer votre demande ultérieurement.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Erreur de confirmation',
				'L\'application mobile ne peut pas charger les détails de cette confirmation. Veuillez redémarrer votre appareil et réessayer plus tard. Si le problème persiste, il est possible qu\'un autre appareil ait été configuré pour fournir les codes Steam Guard pour votre compte, ce qui signifie que l\'authentificateur sur ce téléphone n\'est plus valide.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Annuler la sélection' );
			$btnAccept.text( 'Confirmer la sélection' );
