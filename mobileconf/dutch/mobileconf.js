...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Bevestigingsfout',
					'Er is een probleem opgetreden bij het uitvoeren van die handeling. Probeer je verzoek later opnieuw in te dienen.',
					'Oké'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Bevestigingsfout',
					'Er is een probleem opgetreden bij het annuleren van die bevestiging. Probeer je verzoek later opnieuw in te dienen.',
					'Oké'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Bevestigingsfout',
							typeof responseData.message != 'undefined' ? responseData.message : 'Er is een probleem opgetreden bij het laden van de details van die bevestiging. Probeer je verzoek later opnieuw in te dienen.',
							'Oké'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Bevestigingsfout',
						'Er is een probleem opgetreden bij het laden van de details van die bevestiging. Probeer je verzoek later opnieuw in te dienen.',
						'Oké'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Bevestigingsfout',
				'De mobiele app kan de details voor die bevestiging niet laden. Start je apparaat opnieuw op en probeer je verzoek opnieuw te verzenden. Als je dit probleem blijft ondervinden, kan het zijn dat een ander apparaat is ingesteld om te voorzien in Steam Guard-codes voor je account, waardoor de authenticator op dit apparaat niet langer geldig is.',
				'Oké'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Selectie annuleren' );
			$btnAccept.text( 'Selectie bevestigen' );
