...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Meer tonen' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Onbekend voorwerp ' + itemid, descriptions: [], fraudwarnings: [ 'Er kan geen informatie over dit voorwerp worden gevonden.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Verwijderen uit ruil' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Toevoegen om te ruilen' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Alle #{appname}-voorwerpen');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Deze inventaris is momenteel niet beschikbaar. Probeer het later opnieuw.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Je kunt ze verkrijgen door de gratis drops in het spel, de Mann Co.-winkel in het spel zelf, of door ze met vrienden ruilen."},"620":{"howtoget":"Je kunt ze in de Portal 2-winkel in het spel krijgen of ze met vrienden ruilen."},"753":{"1":{"howtoget":"Je kunt extra spelexemplaren krijgen tijdens speciale aanbiedingen, of door een spel in de Steam-winkel te kopen en \"Als geschenk kopen\" te selecteren bij het betalen."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> van #{contextname}');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Deze hoeveelheid is privé en kun alleen jij zien.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s edelstenen'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Op communitymarkt bekijken' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Actief...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Er is momenteel geen aanbod beschikbaar voor dit item.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '%1$s verkocht in de afgelopen 24 uur';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Mediane prijs: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Aantal: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Verkopen' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Om items te verkopen op de Community-markt moet je <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">saldo toevoegen aan je Steam-portemonnee</a> of een aankoop doen in de Steam-winkel en je factureringsadres opgeven.',
			'Kan item niet verkopen'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Een item te koop aanbieden' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Je moet akkoord gaan met de Steam Subscriber Agreement om dit item te kunnen verkopen.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Je moet een geldige hoeveelheid invoeren.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' De ingevoerde prijs is hoger dan de maximumprijs van %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Je moet een geldige prijs invoeren.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Verkoopprijs bevestigen' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Om dit item op de communitymarkt aan te bieden moet je de vermelding verifiëren in je mobiele Steam-app. Je kunt het verifiëren door de app te starten en vanuit het menu naar de Bevestigingenpagina te navigeren.' +
							'<br><br>' +
							'Als je de optie Bevestigingen niet in het hoofdmenu van de app ziet, controleer dan of je de nieuwste versie van de app hebt.';

				}
				else
				{
					strText = 'Om dit item op de communitymarkt aan te bieden moet je een aanvullende verificatiestap voltooien. Er is een e-mail met aanvullende instructies naar je adres verstuurd (eindigend op "%s").'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'Er is een probleem opgetreden bij het vermelden van je item. Vernieuw de pagina en probeer het opnieuw.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Er is een fout opgetreden bij het opslaan van je selectie. Probeer het later opnieuw.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Mediane verkoopprijzen', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d verkocht'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Gebruik dit om %s te rapporteren aan Steam Support voor een poging tot oplichting.<br><br>Geef een korte beschrijving:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Oplichting melden', $Dialog, 'Oplichting melden').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Oplichting melden', 'Een beschrijving is vereist.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Bedankt!', 'Je melding is verzonden en zal door Steam Support worden beoordeeld.' );
		}).fail( function() {
			ShowAlertDialog( 'Oplichting melden', 'Er is een probleem opgetreden bij het opslaan van je rapport. Probeer het later opnieuw.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Wat is een ruil- of marktblokkering?</div>' );
	description.append( '<div class="escrow_explanation_a">Een blokkering is een tijdsperiode waarin voorwerpen geblokkeerd worden door Steam voordat een ruil voltooid wordt of een marktvermelding geplaatst wordt.</div>' );

	description.append( '<div class="escrow_explanation_q">Items vasthouden beschermt je items</div>' );
	description.append( '<div class="escrow_explanation_a">Steam-accounts zijn waardevol, vooral als ze items hebben die het waard zijn om te stelen. Als je je account niet hebt beveiligd met een fysiek apparaat (de mobiele Steam Guard-authenticator), geeft een ruilblokkering je de tijd om te ontdekken of je account is gekraakt en om te voorkomen dat items je account verlaten.</div>' );

	description.append( '<div class="escrow_explanation_q">Een vertraging om diefstal te onderscheppen en te stoppen</div>' );
	description.append( '<div class="escrow_explanation_a">Wanneer een gebruiker die voorwerpen ruilt of verkoopt zijn account niet ten minste 7 dagen met de mobiele authenticator heeft beveiligd, dan heeft dit deze gevolgen: <br><span style="color: #5aa9d6">Ruilen:</span> het afleveren van voorwerpen die geruild zijn wordt maximaal 15 dagen uitgesteld <br><span style="color: #5aa9d6">Markt:</span> het plaatsen van verkoopvermeldingen wordt maximaal 15 dagen uitgesteld <br><br>Dit geeft de gebruiker de gelegenheid om elke afwachtende transactie die niet door hem is gemachtigd te annuleren. <br><br>Het annuleren van dergelijke transacties of een vastgehouden ruil zal een ruilafkoeltijd op je account in werking stellen om verdere ongemachtigde pogingen tot het ruilen van voorwerpen te voorkomen. Er is geen afkoeltijd voor het annuleren van marktvermeldingen.</div>' );

	description.append( '<div class="escrow_explanation_q">Verwijder de noodzaak voor het vasthouden van items</div>' );
	description.append( '<div class="escrow_explanation_a">Door een mobiele authenticator te gebruiken zorg je ervoor dat jij en alleen jij je voorwerpen snel en veilig kunt ruilen of verkopen, zodat het vasthouden van voorwerpen niet langer nodig is. Verhoog de beveiliging van je account door de <a href="https://store.steampowered.com/mobile/" target="_blank">mobiele Steam Guard-authenticator</a> voor iOS- en Android-apparaten te installeren. </div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Lees het bericht op de Steam-blog over accountbeveiliging</a> voor een gedetailleerde beschrijving over de toevoeging van blokkeringen.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Ruil- en marktblokkeringen',
			description,
			'Meer informatie over de mobiele Steam Guard-authenticator',
			'Sluiten'
		);
