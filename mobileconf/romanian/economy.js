...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Afişează mai mult' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Obiect necunoscut ' + itemid, descriptions: [], fraudwarnings: [ 'Nu s-au putut obţine informaţii despre acest obiect.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Exclude din schimb' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Adaugă la schimb' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Toate obiectele #{appname}');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Acest inventar nu este disponibil în acest moment. Te rugăm să încerci din nou mai târziu.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Le po\u021bi ob\u021bine din c\u0103deri gratuite de obiecte din joc, din Magazinul Mann Co. din joc sau schimb\u00e2nd ceva pentru ele, cu prietenii."},"620":{"howtoget":"Le po\u021bi ob\u021bine de la magazinul din jocul Portal 2 sau schimb\u00e2nd ceva pentru ele, cu prietenii."},"753":{"1":{"howtoget":"Po\u021bi ob\u021bine copii suplimentare de jocuri \u00een timpul promo\u021biilor speciale, sau cump\u0103r\u00e2nd un joc din Magazinul Steam \u0219i select\u00e2nd \"Cump\u0103r drept cadou\" la timpul finaliz\u0103rii."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> din #{contextname}');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Această cantitate este confidenţială şi doar tu o poţi vedea.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s Nestemate'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Vezi în piața comunitară' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Procesare...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Începând de la: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Nu există listări, momentan, pentru acest obiect.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '%1$s vândute în ultimele 24 de ore';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Preț median: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Volum: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Vinde' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Nu poți vinde obiecte pe piața comunitară până când nu <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">adaugi fonduri în portofelul Steam</a> sau realizezi o achiziție din magazinul Steam și furnizezi adresa ta de facturare.',
			'Obiectul nu poate fi vândut'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Pune un obiect la vânzare' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Trebuie să fii de acord cu termenii Acordului pentru abonații Steam pentru a vinde acest obiect.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Trebuie să introduci o cantitate validă.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Prețul introdus depășește prețul maxim de %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Trebuie să introduci un preţ corespunzător.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Confirmă preţul de vânzare' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Pentru a lista acest obiect pe piața comunitară, trebuie să verifici listarea în aplicația pentru mobil Steam. O poți verifica lansând aplicația și navigând la pagina Confirmări din meniu.' +
							'<br><br>' +
							'Dacă nu vezi opțiunea pentru confirmări în meniul principal al aplicației, asigură-te că ai ultima versiune a aplicației.';

				}
				else
				{
					strText = 'Pentru a lista acest obiect în Piața Comunitară, trebuie să completezi o etapă adiţională de verificare. Un e-mail cu informații suplimentare a fost trimis la adresa ta (se termină în "%s").'.replace( /%s/, transport.responseJSON.email_domain );
				}

				ShowAlertDialog(
						'Necesită confirmare în plus',
						strText
				);
			}
			else
		...
		{
			this.DisplayError( 'A apărut o problemă la listarea obiectului. Reîmprospătează pagina şi încearcă din nou.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'A apărut o problemă la salvarea selecției tale, te rugăm să încerci din nou mai târziu.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Prețuri mediane de vânzare', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d vândut'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Folosește acesta pentru a-l raporta pe %s către Asistența Steam pentru tentativă de înșelăciune.<br><br>Te rugăm să oferi o scurtă descriere:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Raportează înşelătorie', $Dialog, 'Raportează înşelătorie').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Raportează înşelătorie', 'O descriere este necesară.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Îți mulțumim!', 'Raportul tău a fost trimis și va fi revizuit de Asistența Steam.' );
		}).fail( function() {
			ShowAlertDialog( 'Raportează înşelătorie', 'A apărut o problemă cu salvarea raportului tău. Te rugăm încearcă din nou mai târziu.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Ce este o reținere a unui schimb sau a unei tranzacții pe piață?</div>' );
	description.append( '<div class="escrow_explanation_a">O reținere constă într-o perioadă de timp în care obiectele sunt reținute de Steam înainte ca un schimb să fie finalizat sau când este postată o listare pe piață.</div>' );

	description.append( '<div class="escrow_explanation_q">Schimburile reținute te ajută să îți protejezi obiectele</div>' );
	description.append( '<div class="escrow_explanation_a">Conturile Steam sunt valoroase, mai ales dacă ai obiecte ce merită furate. Dacă nu ți-ai protejat contul cu un dispozitiv fizic (Autentificatorul Steam Guard pentru Mobil), o reținere a schimbului îți va aloca timp să descoperi dacă contul tău a fost compromis și pentru a preveni sustragerea obiectelor din contul tău.</div>' );

	description.append( '<div class="escrow_explanation_q">O întârziere pentru a prinde și a opri furtul de obiecte</div>' );
	description.append( '<div class="escrow_explanation_a">Dacă un utilizator dă la schimb sau vinde obiectele sale pe piață și nu are contul protejat de un autentificator pentru mobil de cel puțin 7 zile: <br><span style="color: #5aa9d6">Schimburi:</span> livrarea obiectelor de la schimburile finalizate vor fi amânate de Steam timp de cel mult 15 zile <br><span style="color: #5aa9d6">Piață:</span> listările de vânzare vor fi reținute de Steam înainte să fie postate timp de cel mult 15 zile <br><br>Astfel, se oferă utilizatorului timpul necesar pentru a anula tranzacțiile în curs, pe care nu le-a autorizat.<br><br>Anularea schimburilor care sunt în curs sau reținute va declanșa o perioadă de așteptare pe contul tău, care va preveni alte încercări neautorizate de a-ți da la schimb obiectele. Nu există o asemenea perioadă pentru anularea listărilor din piață.</div>' );

	description.append( '<div class="escrow_explanation_q">Înlătură necesitatea pentru schimburi reținute</div>' );
	description.append( '<div class="escrow_explanation_a">Folosirea unui Autentificator pentru Mobil îți asigură că doar tu, și numai tu, poți da la schimb sau vinde obiectele în mod rapid și sigur, astfel încât reținerile de schimburi nu mai sunt necesare. Sporește-ți securitatea contului prin descărcarea <a href="https://store.steampowered.com/mobile/" target="_blank">autentificatorului Steam Guard pentru mobil</a> pentru dispozitive iOS și Android. </div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Citește postarea de pe blogul Steam despre securitatea contului</a> pentru o explicație detaliată în legătură cu motivul pentru care am implementat reținerile.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Rețineri de schimburi și tranzacții pe piață',
			description,
			'Află mai multe despre Autentificatorul Steam Guard pentru mobil',
			'Închide'
		);
