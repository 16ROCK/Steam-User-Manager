...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Eroare la confirmare',
					'A apărut o problemă la executarea acelei acțiuni. Te rugăm să încerci din nou mai târziu.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Eroare la confirmare',
					'A fost o problemă cu anularea confirmării. Te rugăm să încerci din nou mai târziu.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Eroare la confirmare',
							typeof responseData.message != 'undefined' ? responseData.message : 'A fost o problemă cu încărcarea detaliilor pentru această confirmare. Te rugăm să încerci mai târziu.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Eroare la confirmare',
						'A fost o problemă cu încărcarea detaliilor pentru această confirmare. Te rugăm să încerci mai târziu.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Eroare la confirmare',
				'Aplicația pentru mobil nu poate să încarce detalii cu privire la acea confirmare. Te rugăm să repornești dispozitivul și să efectuezi solicitarea din nou mai târziu. Dacă întâmpini în continuare probleme, s-ar putea ca un alt dispozitiv să fie setat pentru a genera coduri Steam Guard pentru contul tău, ceea ce înseamnă că autentificatorul de pe acest dispozitiv nu mai este valid.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Anulează selecția' );
			$btnAccept.text( 'Confirmă selecția' );
