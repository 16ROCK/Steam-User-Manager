...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'确认错误',
							typeof responseData.message != 'undefined' ? responseData.message : '载入该确认详情时遇到问题。请稍后再重试您的请求。',
							'确定'
					).always( function() {
						window.history.back();
					} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'确认错误',
				'移动应用无法加载该确认的详细信息。请重启您的设备，然后重试您的请求。如果您仍然遇到问题，可能是因为您已经设置其他设备为您的帐户提供 Steam 令牌代码，使该设备上的验证器失效。',
				'确定'
			).always( function() {
				window.history.back();
			} );
