...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Bekräftelsefel',
					'Det uppstod ett problem när åtgärden skulle utföras. Försök igen senare.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Bekräftelsefel',
					'Det uppstod ett fel vid avbrytningen av bekräftelsen. Försök igen senare.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Bekräftelsefel',
							typeof responseData.message != 'undefined' ? responseData.message : 'Det uppstod ett problem när bekräftelsedetaljerna skulle läsas in. Försök igen senare.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Bekräftelsefel',
						'Det uppstod ett problem när bekräftelsedetaljerna skulle läsas in. Försök igen senare.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Bekräftelsefel',
				'Mobilappen kunde inte läsa in bekräftelsens detaljer. Starta om din enhet och försök igen senare med din begäran. Om du fortfarande stöter på problem, kan det bero på att en annan enhet har upprättas för att skicka Steam Guard-koder till ditt konto, vilket betyder att verifieraren på denna enhet inte längre är giltig.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Avbryt vald' );
			$btnAccept.text( 'Bekräfta vald' );
