...
var CInventory = Class.create( {
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Okänt föremål ' + itemid, descriptions: [], fraudwarnings: [ 'Kunde inte hämta information om detta föremål.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Ta bort från byte' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Lägg till byte' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Alla #{appname}-föremål');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Detta förråd är för tillfället otillgängligt. Försök igen senare.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Du kan f\u00e5 dem gratis slumpm\u00e4ssigt i spelet, i Mann Co.-butiken eller byta till dig dem med v\u00e4nner."},"620":{"howtoget":"Du kan f\u00e5 dem fr\u00e5n Portal 2-butiken eller byta till dig dem med v\u00e4nner."},"753":{"1":{"howtoget":"Du kan f\u00e5 extra kopior av spel under speciella kampanjer, eller genom att k\u00f6pa ett spel fr\u00e5n Steam-Butiken och v\u00e4lja \"K\u00f6p som g\u00e5va\"  n\u00e4r du checkar ut."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> från #{contextname}');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Detta antal är privat och visas endast för dig.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s ädelstenar'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Visa i Gemenskapsmarknad' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Bearbetar...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Pris från: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Det finns just nu inga annonser för den här artikeln.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '%1$s sålda de senaste 24 timmarna';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Medianpris: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Mängd: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Sälj' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Du kan inte sälja den här artikeln på gemenskapsmarknaden innan du har <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">satt in pengar i din Steam-plånbok</a> eller genomfört ett köp i Steam-butiken och angett din faktureringsadress.',
			'Kan inte sälja artikeln'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Lägg ut en artikel till försäljning' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Du måste godkänna villkoren i Steams abonnentavtal för att få sälja den här artikeln.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Du måste uppge en giltig mängd.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Det angivna priset överskrider det högsta priset på %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Du måste ange ett giltigt pris.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Bekräfta säljpriset' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'För att lägga upp denna artikel på gemenskapsmarknaden, måste du verifiera annonsen i Steams mobilapp. Du kan verifiera den genom att starta appen och navigera till bekräftelsesidan från menyn.' +
							'<br><br>' +
							'Om du inte ser alternativet Bekräftelser i appens huvudmeny, se till att du har den senaste versionen av appen.';

				}
				else
				{
					strText = 'För att lägga upp denna artikel på gemenskapsmarknaden, måste du slutföra ett till verifikationssteg. Ett e-postmeddelande har skickats till din adress (slutar på "%s") med ytterligare instruktioner.'.replace( /%s/, transport.responseJSON.email_domain );
				}

				ShowAlertDialog(
						'Ytterligare bekräftelse krävs',
						strText
				);
			}
			else
		...
		{
			this.DisplayError( 'Ett problem uppstod när din artikel annonserades ut. Uppdatera sidan och försök igen.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Ett fel uppstod när ditt urval sparades, försök igen senare.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Mediansäljspris', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d såld(a)'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Använd det här för att anmäla %s till Steam Support för bedrägeriförsök.<br><br>Var god förse oss med en kortfattad beskrivning:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Anmäl bedrägeri', $Dialog, 'Anmäl bedrägeri').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Anmäl bedrägeri', 'En beskrivning krävs.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Tack!', 'Din anmälan har skickats in och kommer granskas av Steam Support.' );
		}).fail( function() {
			ShowAlertDialog( 'Anmäl bedrägeri', 'Det uppstod ett problem när din anmälan skulle sparas. Var god försök igen senare.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Vad är reservering av föremål vid byten och marknadsannonseringar?</div>' );
	description.append( '<div class="escrow_explanation_a">En reservering är då föremål reserveras av Steam i en viss tidsperiod innan ett byte slutförs, eller en marknadsannons läggs ut.</div>' );

	description.append( '<div class="escrow_explanation_q">Reservering av föremål medverkar till att skydda dina föremål från stöld</div>' );
	description.append( '<div class="escrow_explanation_a">Steam-konton är värdefulla, speciellt om de har föremål som är värda att stjäla. Om du inte har skyddat ditt konto med en fysisk enhet (med mobilautentiseraren för Steam Guard) kommer reserveringen av föremål ge dig tiden att upptäcka att ditt konto har kapats, och förhindra att dina föremål försvinner från ditt konto.</div>' );

	description.append( '<div class="escrow_explanation_q">En fördröjning för att upptäcka och hindra artikelstöld</div>' );
	description.append( '<div class="escrow_explanation_a">Om en användare som byter eller säljer föremål inte haft sitt konto skyddat av en mobilautentiserare under de senaste 7 dagarna:<br><span style="color: #5aa9d6">Byten:</span> Föremål från slutförda byten kommer att fördröjas av Steam i upp till 15 dagar innan de kommer fram <br><span style="color: #5aa9d6">Marknad:</span> Säljannonser kommer att fördröjas av Steam innan de läggs ut, i upp till 15 dagar <br><br>Detta ger användaren tid att avbryta pågående transaktioner han/hon inte godkänt. <br><br>Att avbryta byten som pågår eller är reserverade av Steam kommer aktivera en tidsbegränsad spärr på ditt konto för att förhindra att ytterligare obehöriga byten sker. Det resulterar inte i någon tidsbegränsad spärr om man avbryter marknadsannonser.</div>' );

	description.append( '<div class="escrow_explanation_q">Ta bort behovet för reservering av föremål</div>' );
	description.append( '<div class="escrow_explanation_a">Användandet av en mobilautentiserare försäkrar att enbart du kan byta eller sälja dina föremål snabbt och säkert, så reservation av föremål inte längre krävs. Öka din kontosäkerhet genom att hämta <a href="https://store.steampowered.com/mobile/" target="_blank">Steam Guard Mobile Authenticator</a> för iOS- och Android-enheter.</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Läs Steams bloggpost om kontosäkerhet</a> för en mer ingående förklaring på varför vi införde reservation av föremål.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Reservering av föremål',
			description,
			'Läs mer om Steam Guards mobilautentiserare',
			'Stäng'
		);
	...
	else
	{
		var dialog = ShowAlertDialog(
			'Reservering av föremål i byten',
			description
		);
	}
	return false;
