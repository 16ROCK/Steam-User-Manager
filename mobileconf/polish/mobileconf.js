...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Błąd potwierdzenia',
					'Wystąpił błąd podczas wykonywania tej czynności. Spróbuj ponownie później.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Błąd potwierdzenia',
					'Wystąpił błąd podczas anulowania tego potwierdzania. Spróbuj ponownie później.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Błąd potwierdzenia',
							typeof responseData.message != 'undefined' ? responseData.message : 'Wystąpił błąd podczas wczytywania szczegółów tego potwierdzenia. Spróbuj ponownie później.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Błąd potwierdzenia',
						'Wystąpił błąd podczas wczytywania szczegółów tego potwierdzenia. Spróbuj ponownie później.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Błąd potwierdzenia',
				'Mobilna aplikacja nie może wczytać szczegółów tego potwierdzenia. Uruchom ponownie swoje urządzenie i spróbuj potwierdzić ponownie później. Jeżeli wciąż masz ten sam problem, to możliwe, że inne urządzenie zostało skonfigurowane do wysyłania kodów Steam Guard dla twojego konta, co oznacza, że token uwierzytelniający znajdujący się na tym urządzeniu utracił ważność.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Anuluj zaznaczone' );
			$btnAccept.text( 'Potwierdź zaznaczone' );
