...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Pokaż więcej' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Nieznany przedmiot ' + itemid, descriptions: [], fraudwarnings: [ 'Nie można było zdobyć informacji o tym przedmiocie.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Usuń z wymiany' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Dodaj do wymiany' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Wszystkie przedmioty - #{appname}');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Ten ekwipunek jest aktualnie niedostępny. Spróbuj ponownie później.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Mo\u017cesz zdoby\u0107 je podczas gry, w Sklepie Mann Co. lub otrzyma\u0107 od znajomych."},"620":{"howtoget":"Mo\u017cesz zdoby\u0107 je w sklepie w grze Portal 2 lub otrzyma\u0107 od znajomych."},"753":{"1":{"howtoget":"Mo\u017cesz zdoby\u0107 dodatkowe egzemplarze gier podczas specjalnych promocji albo zamawiaj\u0105c gr\u0119 w Sklepie Steam, zaznaczaj\u0105c \u201eKup jako prezent\u201d w podsumowaniu transakcji."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Ta liczba jest prywatna i widoczna tylko dla ciebie.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s szt. klejnotów'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Pokaż na Rynku Społeczności' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Praca w toku...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Już od: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Nie ma obecnie ofert sprzedaży dla tego przedmiotu.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = 'Sprzedano %1$s przez ostatnie 24 godziny';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Mediana cen: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Nakład: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Sprzedaj' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Przedmioty można sprzedawać na Rynku Społeczności Steam dopiero po <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">dodaniu środków do Portfela Steam</a> lub dokonaniu zakupu w Sklepie Steam i podaniu swojego adresu rozliczeniowego.',
			'Nie można sprzedać przedmiotu'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Wystaw przedmiot na sprzedaż' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Musisz wyrazić zgodę na warunki Umowy użytkownika Steam, by sprzedać ten przedmiot.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Musisz podać prawidłową ilość.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Podana cena przekracza maksymalną wartość wynoszącą %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Musisz podać poprawną cenę.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Potwierdź cenę sprzedaży' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Aby umieścić ten przedmiot na Rynku Społeczności, musisz zweryfikować tę ofertę w swojej aplikacji mobilnej Steam. Możesz tego dokonać poprzez uruchomienie aplikacji, a następnie wejście w zakładkę Potwierdzenia z menu.' +
							'<br><br>' +
							'Upewnij się, że masz najnowszą wersję aplikacji, jeśli nie widzisz opcji potwierdzenia w głównym menu.';

				}
				else
				{
					strText = 'Aby wystawić ten przedmiot na Rynku Społeczności, musisz dokonać dodatkowej weryfikacji. Wysłaliśmy e-mail na twój adres (kończący się na „%s”) zawierający dalsze instrukcje.'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'Wystąpił błąd podczas wystawiania przedmiotu. Odśwież stronę i spróbuj ponownie.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Wystąpił problem podczas zapisywania twojego wyboru. Spróbuj ponownie później.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Mediana cen sprzedaży', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>Sprzedanych: %d'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Użyj tego, aby zgłosić %s do Pomocy technicznej Steam za próbę oszustwa.<br><br>Opisz krótko sytuację:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Zgłoś oszustwo', $Dialog, 'Zgłoś oszustwo').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Zgłoś oszustwo', 'Wymagany jest opis.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Dziękujemy!', 'Twoje zgłoszenie zostało wysłane i zostanie przejrzane przez Pomoc techniczną Steam.' );
		}).fail( function() {
			ShowAlertDialog( 'Zgłoś oszustwo', 'Wystąpił problem podczas zapisywania twojego zgłoszenia. Spróbuj ponownie później.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Czym jest wstrzymanie wymiany lub transakcji na rynku?</div>' );
	description.append( '<div class="escrow_explanation_a">Wstrzymanie to okres czasu, w którym przedmioty są przetrzymywane przez Steam, zanim zakończy się wymiana lub zanim umieszczone zostanie ogłoszenie na rynku.</div>' );

	description.append( '<div class="escrow_explanation_q">Wstrzymanie przedmiotów pomaga chronić twój ekwipunek</div>' );
	description.append( '<div class="escrow_explanation_a">Konta Steam są cenne, szczególnie jeśli znajdują się na nich przedmioty o dużej wartości. Jeśli twoje konto Steam nie zostało zabezpieczone przy użyciu jakiegoś urządzenia (poprzez mobilny token uwierzytelniający Steam Guard), wstrzymanie wymiany da ci czas na odkrycie, że bezpieczeństwo konta jest zagrożone, dzięki czemu uda się zapobiec kradzieży przedmiotów.</div>' );

	description.append( '<div class="escrow_explanation_q">Opóźnienie mające na celu wyłapanie i zatrzymanie kradzieży przedmiotów</div>' );
	description.append( '<div class="escrow_explanation_a">Jeśli użytkownik wymieniający lub sprzedający przedmioty nie chronił swojego konta przez 7 ostatnich dni mobilnym tokenem uwierzytelniającym:\n<br><span style="color: #5aa9d6">Wymiany:</span> dostarczenie przedmiotów po zaakceptowaniu wymiany zostanie opóźnione przez Steam nawet do 15 dni.\n<br><span style="color: #5aa9d6">Rynek:</span> oferty sprzedaży będą wstrzymywane przez Steam przed ich zamieszczeniem nawet do 15 dni.<br>\n<br>To zapewnia użytkownikowi czas na odrzucenie jakichkolwiek oczekujących transakcji, których nie uwierzytelnił.<br>\n<br>Odwołanie wymian, które są oczekujące lub wstrzymane, wywoła okres ich blokady na twoim koncie, by zapobiec dalszym nieautoryzowanym próbom wymian twoich przedmiotów. Blokady nie są nakładane za odwołanie ogłoszeń na rynku.</div>' );

	description.append( '<div class="escrow_explanation_q">Zlikwiduj potrzebę wstrzymania wymiany</div>' );
	description.append( '<div class="escrow_explanation_a">Dzięki używaniu mobilnego tokena uwierzytelniającego będziesz mieć pewność szybkiego i bezpiecznego dokonywania wymian. Nie będzie wtedy potrzeby wprowadzania blokad handlu. Zwiększ bezpieczeństwo swojego konta poprzez aktywację <a href="https://store.steampowered.com/mobile/" target="_blank">mobilnego tokena uwierzytelniającego Steam Guard</a> dla urządzeń działających na systemach Android, iOS oraz Windows Phone. </div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Przeczytaj post na blogu Steam o bezpieczeństwie konta</a>, by dowiedzieć się więcej o powodach wprowadzenia funkcji wstrzymania wymian.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Wstrzymania wymian i transakcji na rynku',
			description,
			'Dowiedz się więcej o mobilnym tokenie uwierzytelniającym Steam Guard',
			'Zamknij'
		);
