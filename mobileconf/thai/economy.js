	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'ช่องเก็บของนี้ไม่พร้อมใช้งานอยู่ในขณะนี้ กรุณาลองใหม่อีกครั้งภายหลัง' );
			}
		}
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="กำลังดำเนินการ...">' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'คุณไม่สามารถขายไอเท็มในตลาดชุมชนได้จนกว่าคุณจะ <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">เพิ่มเงินเข้าวอลเล็ต Steam ของคุณ</a> หรือทำการสั่งซื้อในร้านค้า Steam และให้ที่อยู่ในใบเสร็จของคุณ',
			'ไม่สามารถขายไอเท็มได้'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'นำไอเท็มขึ้นวางขาย' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'คุณต้องยอมรับข้อกำหนดของข้อตกลงการสมัครสมาชิกของ Steam เพื่อขายไอเท็มนี้' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'คุณต้องกรอกจำนวนอย่างถูกต้อง' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' ราคาที่กรอกไว้เกินราคาสูงสุดของ %1$s'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'คุณต้องกรอกราคาให้ถูกต้อง' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'ยืนยันราคาขาย' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'เพื่อตั้งรายการขายไอเท็มนี้บนตลาดชุมชน คุณต้องยืนยันการตั้งรายการขายในแอป Steam แบบพกพาของคุณ คุณสามารถยืนยันโดยการเปิดแอปและไปยังหน้าการยืนยันได้จากเมนู' +
							'<br><br>' +
							'หากคุณไม่เห็นตัวเลือกการยืนยันในเมนูหลักของแอป กรุณาตรวจสอบให้แน่ใจว่าคุณมีแอปเวอร์ชันล่าสุดแล้ว';

				}
				else
				{
					strText = 'เพื่อตั้งรายการขายไอเท็มนี้บนตลาดชุมชน คุณต้องดำเนินการตามขั้นตอนการตรวจสอบเพิ่มเติมให้เสร็จสมบูรณ์ก่อน อีเมลได้ถูกส่งไปยังที่อยู่ของคุณแล้ว (ลงท้ายด้วย "%s") พร้อมกับคำแนะนำเพิ่มเติม'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'ตรวจพบปัญหาในรายการขายไอเท็มของคุณ รีเฟรชหน้านี้และลองใหม่อีกครั้ง' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'ตรวจพบปัญหาในการบันทึกการเลือกของคุณ กรุณาลองใหม่อีกครั้งภายหลัง' );
	}
}
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'ใช้สิ่งนี้เพื่อรายงาน %s ไปยังฝ่ายสนับสนุนของ Steam สำหรับความพยายามในการหลอกลวง <br><br>กรุณากรอกรายละเอียดโดยสรุป:'.replace( /%s/, strPersonaName ) ) );
	...
	var Modal = ShowConfirmDialog( 'รายงานการหลอกลวง', $Dialog, 'รายงานการหลอกลวง').done( function() {
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			...
			ShowAlertDialog( 'รายงานการหลอกลวง', 'ตรวจพบปัญหาในการบันทึกรายงานของคุณ กรุณาลองใหม่อีกครั้งภายหลัง' );
		});
	} );
