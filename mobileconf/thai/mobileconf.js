...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'ข้อผิดพลาดในการยืนยัน',
					'ตรวจพบปัญหาในการกระทำการนั้น กรุณาลองส่งคำร้องของคุณอีกครั้งภายหลัง',
					'ตกลง'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'ข้อผิดพลาดในการยืนยัน',
					'ตรวจพบปัญหาในการยกเลิกการยืนยันนั้น กรุณาลองส่งคำขอของคุณอีกครั้งภายหลัง',
					'ตกลง'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'ข้อผิดพลาดในการยืนยัน',
							typeof responseData.message != 'undefined' ? responseData.message : 'ตรวจพบปัญหาในการโหลดรายละเอียดสำหรับการยืนยันนั้น กรุณาลองส่งคำร้องของคุณอีกครั้งภายหลัง',
							'ตกลง'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'ข้อผิดพลาดในการยืนยัน',
						'ตรวจพบปัญหาในการโหลดรายละเอียดสำหรับการยืนยันนั้น กรุณาลองส่งคำร้องของคุณอีกครั้งภายหลัง',
						'ตกลง'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'ข้อผิดพลาดในการยืนยัน',
				'แอปแบบพกพาไม่สามารถโหลดรายละเอียดสำหรับการยืนยันนั้นได้ กรุณารีบูตอุปกรณ์ของคุณและลองคำร้องของคุณอีกครั้งภายหลัง หากคุณยังคงกำลังประสบปัญหา อาจเป็นไปได้ว่าอุปกรณ์อื่นได้ถูกตั้งค่าไว้เพื่อมอบรหัส Steam Guard สำหรับบัญชีของคุณแทน ซึ่งหมายความว่าเครื่องยืนยันตัวตนบนอุปกรณ์นี้ไม่สามารถใช้งานได้อีกต่อไป',
				'ตกลง'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'ยกเลิกการเลือก' );
			$btnAccept.text( 'ยืนยันการเลือก' );
