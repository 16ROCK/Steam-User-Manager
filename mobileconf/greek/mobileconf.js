...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Σφάλμα επιβεβαίωσης',
					'Υπήρξε ένα πρόβλημα με την εκτέλεση της ενέργειας. Παρακαλούμε δοκιμάστε το αίτημά σας ξανά αργότερα.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Σφάλμα επιβεβαίωσης',
					'Υπήρξε ένα πρόβλημα με την ακύρωση της επιβεβαίωσης. Παρακαλούμε δοκιμάστε το αίτημά σας ξανά αργότερα.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Σφάλμα επιβεβαίωσης',
							typeof responseData.message != 'undefined' ? responseData.message : 'Υπήρξε ένα πρόβλημα με τη φόρτωση λεπτομερειών για την επιβεβαίωση. Παρακαλούμε δοκιμάστε το αίτημά σας ξανά αργότερα.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Σφάλμα επιβεβαίωσης',
						'Υπήρξε ένα πρόβλημα με τη φόρτωση λεπτομερειών για την επιβεβαίωση. Παρακαλούμε δοκιμάστε το αίτημά σας ξανά αργότερα.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Σφάλμα επιβεβαίωσης',
				'Η εφαρμογή για κινητά αδυνατεί να φορτώσει πληροφορίες για αυτή την επιβεβαίωση. Παρακαλούμε επανεκκινήστε τη συσκευή σας και προσπαθήστε το αίτημά σας ξανά αργότερα. Αν εξακολουθείτε να αντιμετωπίζετε πρόβλημα, μπορεί να συμβαίνει επειδή μια άλλη συσκευή έχει οριστεί ως πάροχος κωδικών Steam Guard για το λογαριασμό σας, που σημαίνει πως ο επαληθευτής σε αυτή τη συσκευή δεν είναι πλέον έγκυρος.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Ακύρωση επιλεγμένων' );
			$btnAccept.text( 'Επιβεβαίωση επιλεγμένων' );
