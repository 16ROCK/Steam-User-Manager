...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Vahvistusvirhe',
					'Toiminnon suorittamisessa tapahtui virhe. Yritä myöhemmin uudelleen.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Vahvistusvirhe',
					'Vahvistuksen peruuttamisessa tapahtui virhe. Yritä myöhemmin uudelleen.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Vahvistusvirhe',
							typeof responseData.message != 'undefined' ? responseData.message : 'Vahvistuksen tietojen latauksessa tapahtui virhe. Yritä myöhemmin uudelleen.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Vahvistusvirhe',
						'Vahvistuksen tietojen latauksessa tapahtui virhe. Yritä myöhemmin uudelleen.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Vahvistusvirhe',
				'Mobiilisovellus ei saa ladattua vahvistuksen yksityiskohtia. Käynnistä laitteesi uudelleen ja yritä pyyntöä myöhemmin uudelleen. Mikäli ongelma jatkuu, se voi tarkoittaa, että toinen laite on asetettu lähettämään Steam Guard -koodeja tilillesi, mikä tarkoittaa, että parhaillaan käyttämäsi laitteen varmenne ei ole enää voimassa.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Peruuta valitut' );
			$btnAccept.text( 'Vahvista valitut' );
