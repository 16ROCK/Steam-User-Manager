...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Näytä lisää' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Tuntematon esine ' + itemid, descriptions: [], fraudwarnings: [ 'Esineen tietojen haku epäonnistui.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					if ( bInTrade )
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Poista vaihtokaupasta' );
					}
					else
					{
						$BtnAddToTrade.removeClass('btn_grey_white_innerfade' ).addClass('btn_green_white_innerfade');
						$BtnAddToTrade.children('span' ).text( 'Lisää vaihtoon' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Kaikki #{appname} -esineet');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Tämä tavaraluettelo ei ole saatavilla tällä hetkellä. Yritä myöhemmin uudelleen.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Voit saada esineit\u00e4 ilmaiseksi pelin satunnaisista esinel\u00f6yd\u00f6ist\u00e4, pelinsis\u00e4isest\u00e4 Mann Co. -kaupasta, tai vaihdossa kavereiltasi."},"620":{"howtoget":"Voit hankkia niit\u00e4 Portal 2:n pelinsis\u00e4isest\u00e4 kaupasta tai saada vaihdossa kavereiltasi."},"753":{"1":{"howtoget":"Voit saada ylim\u00e4\u00e4r\u00e4isi\u00e4 kopioita peleist\u00e4 promootiotapahtumien aikana, tai ostamalla pelin Steam-kaupasta ja valitsemalla \"Osta lahjaksi\" maksaessasi ostoksiasi."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> (#{contextname})');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Tämä määrä on yksityinen ja näytetään vain sinulle.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s jalokiveä'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Tarkastele Yhteisön kauppapaikalla' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Käsitellään...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Alin hinta: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Tälle esineelle ei tällä hetkellä ole listauksia.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '%1$s myyty viimeisen 24 tunnin aikana';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += 'Mediaanihinta: ' + transport.responseJSON.median_price + '<br>';
							strInfo += 'Määrä: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Myy' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Et voi myydä esineitä yhteisön kauppapaikalla, ennen kuin <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">lisäät saldoa Steam-lompakkoosi</a> tai teet ostoksen Steam-kaupassa ja annat laskutusosoitteesi.',
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Laita esine myyntiin' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Sinun on hyväksyttävä Steam-tilaussopimuksen ehdot, jos haluat myydä tämän esineen.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Sinun on annettava kunnollinen määrä.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Annettu hinta on suurempi kuin maksimihinta (%1$s).'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Anna kelvollinen hinta.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Vahvista myyntihinta' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Voidaksesi listata tämän tuotteen yhteisön kauppapaikkaan, sinun täytyy vahvistaa listaus Steam-mobiilisovelluksella. Vahvistaaksesi listauksen, käynnistä sovellus ja valitse valikosta vahvistukset-sivu.' +
							'<br><br>' +
							'Jos et näe sovelluksen päävalikossa Vahvistukset-vaihtoehtoa varmistathan, että käytössäsi on sovelluksen uusin päivitys.';

				}
				else
				{
					strText = 'Voidaksesi listata tämän esineen Yhteisön kauppapaikkaan, sinun tulee vahvistaa listauksesi. Lisäohjeita sisältävä sähköposti on lähetetty osoitteesesi (joka päättyy "%s").'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( 'Ongelma esineen listauksessa. Päivitä sivu ja yritä uudelleen.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Valintaasi ei voitu tallentaa, ole hyvä ja yritä myöhemmin uudelleen.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Myyntihintojen mediaani', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d myyty'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Käytä tätä ilmoittaaksesi henkilön %s Steamin tuelle huijausyrityksestä.<br><br>Ole hyvä ja anna lyhyt kuvaus tapahtuneesta:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Ilmoita huijaus', $Dialog, 'Ilmoita huijaus').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Ilmoita huijaus', 'Kuvaus vaaditaan.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Kiitos!', 'Ilmoituksesi on lähetetty ja se käsitellään Steamin tuessa.' );
		}).fail( function() {
			ShowAlertDialog( 'Ilmoita huijaus', 'Ilmoituksesi tallentaminen epäonnistui. Yritä myöhemmin uudelleen.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Mitä ovat vaihtojen ja kauppapaikan jonot?</div>' );
	description.append( '<div class="escrow_explanation_a">Jonotus on aika, jonka esineet odottavat Steamissä ennen vaihdon loppuunsaattamista tai kauppapaikan listauksen lähetystä.</div>' );

	description.append( '<div class="escrow_explanation_q">Vaihtojonot auttavat suojelemaan esineitäsi</div>' );
	description.append( '<div class="escrow_explanation_a">Steam-tilit ovat arvokkaita ja erityisesti silloin, jos ne sisältävät varastamisen arvoisia esineitä. Jos et ole suojannut tiliäsi fyysisellä laitteella (Steam Guard -mobiilivarmenne), vaihtojonotus antaa sinulle aikaa huomata tilisi kaappauksen ja estää esineiden häviämisen tililtäsi.</div>' );

	description.append( '<div class="escrow_explanation_q">Viive esinevarkauden estämiseksi</div>' );
	description.append( '<div class="escrow_explanation_a">Jos käyttäjä vaihtaa tai myy esineitä ilman, että tili on ollut suojattuna Steam Guard -varmenteella 7 päivän ajan:<br>\n<span style="color: #5aa9d6">Vaihdot:</span> hyväksyttyjen vaihtoesineiden toimitus viivästyy Steamissä enintään 15 päiväksi<br>\n<span style="color: #5aa9d6">Kauppapaikka:</span> myyntilistaukset siirtyvät jonotukseen enintään 15 päiväksi ennen lähettämistä<br><br>\nTämä antaa käyttäjälle aikaa peruuttaa kaikki luvattomat vaihdot, myynnit tai ostot.<br><br>\nJonotuksessa olevien vaihtojen peruuttamisesta seuraa vaihtojäähy, joka estää mahdolliset, tulevat ja luvattomat vaihdot tililtä. Kauppapaikan listauksen peruuttamisesta ei seuraa jäähyä.</div>' );

	description.append( '<div class="escrow_explanation_q">Poista vaihtojonotuksen tarve</div>' );
	description.append( '<div class="escrow_explanation_a">Käyttämällä mobiilivarmennetta varmistat, että vain sinä voit vaihtaa tai myydä esineitäsi nopeasti ja turvallisesti, joten vaihtojonotuksia ei enää tarvita. Lisää tilisi turvallisuutta hankkimalla <a href="https://store.steampowered.com/mobile/" target="_blank">Steam Guard -mobiilivarmenne</a> iOS- ja Android -laitteille.</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Lue Steam-blogista viestimme tilien turvallisuudesta</a> saadaksesi tarkemman selityksen vaihtojonotuksien käyttöönoton syistä.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Vaihtojen ja kauppapaikan jonot',
			description,
			'Lisätietoja Steam Guard -mobiilivarmenteesta',
			'Sulje'
		);
