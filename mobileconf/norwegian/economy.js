...
var CInventory = Class.create( {
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Ukjent gjenstand ' + itemid, descriptions: [], fraudwarnings: [ 'Kunne ikke hente informasjon om denne gjenstanden.' ] };
			...
			if ( bUserAction )
			{
				...
				if ( $BtnAddToTrade.length && typeof OnDoubleClickItem != 'undefined' )
				{
					...
					else
					{
						...
						$BtnAddToTrade.children('span' ).text( 'Legg til i byttehandel' );
					}
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Alle gjenstander for #{appname}');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Dette lageret er ikke tilgjengelig akkurat nå. Prøv igjen senere.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"Du kan f\u00e5 dem fra gratis gjenstandsdropp i spillet, Mann Co.-butikken i spillet eller byttehandle for dem med venner."},"620":{"howtoget":"Du kan kj\u00f8pe dem fra Portal 2-butikken i spillet eller byttehandle for dem med venner."},"753":{"1":{"howtoget":"Du kan f\u00e5 ekstra eksemplarer av spill under spesielle kampanjer, eller ved \u00e5 kj\u00f8pe et spill fra Steam-butikken og velge \"Kj\u00f8p som gave\" ved kassen."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Dette beløpet er privat og vises bare til deg.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s edelsteiner'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Vis i samfunnsmarkedet' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="Arbeider …">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += 'Fra: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += 'Det er for øyeblikket ingen oppføringer tilgjengelige for denne gjenstanden.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '%1$s solgt de siste 24 timene';
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Selg' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Du kan ikke selge gjenstander på markedet for Steam-samfunnet før du <a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">legger til midler i Steam-lommeboken</a> eller kjøper noe i Steam-butikken og oppgir en fakturaadresse.',
			'Kan ikke selge gjenstand'
		);
	}
	else
	{
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Legg ut en gjenstand for salg' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Du må godta betingelsen i Steams abonnementsavtale for å kunne selge denne gjenstanden.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Du må skrive inn et gyldig antall.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Prisen som er angitt, overskrider maksimalverdien på %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Du må angi en gyldig pris.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Bekreft salgspris' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'For å legge ut denne gjenstanden på samfunnsmarkedet, må du først bekrefte at du vil legge den ut med Steam-appen. Du kan bekrefte det ved å starte appen og gå til «Bekreftelser» fra menyen.' +
							'<br><br>' +
							'Hvis du ikke ser Bekreft-alternativet i hovedmenyen i appen, må du sjekke om du har den nyeste versjonen av appen.';

				}
				else
				{
					strText = 'Du må gjennomføre et ekstra bekreftelsestrinn for å annonsere denne gjenstanden på samfunnsmarkedet. En e-post er sendt til adressen din (som slutter på «%s») med ytterligere instruksjoner.'.replace( /%s/, transport.responseJSON.email_domain );
				}

				ShowAlertDialog(
						'Ytterligere bekreftelse trengs',
						strText
				);
			}
			else
		...
		{
			this.DisplayError( 'Det oppstod et problem med å legge inn annonsen for gjenstanden. Oppdater siden, og prøv på nytt.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Det oppstod et problem med å lagre valgene. Prøv igjen senere.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Median for salgspriser', textAlign: 'left' },
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Bruk dette til å rapportere %s til Steams kundestøtte for svindelforsøk.<br><br>Gi oss også en kort beskrivelse:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Rapporter svindel', $Dialog, 'Rapporter svindel').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Rapporter svindel', 'En beskrivelse kreves.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Takk!', 'Rapporten din er blitt sendt inn og vil bli gjennomgått av Steams kundestøtte.' );
		}).fail( function() {
			ShowAlertDialog( 'Rapporter svindel', 'Det oppstod en feil ved lagring av rapporten. Prøv igjen senere.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Hva er bytte- eller markedsoppbevaring?</div>' );
	description.append( '<div class="escrow_explanation_a">Oppbevaring er en tidsperiode der gjenstandene blir oppbevart av Steam før en byttehandel blir fullført, eller en annonse blir opprettet på markedet.</div>' );

	description.append( '<div class="escrow_explanation_q">Gjenstandsoppbevaring bidrar til å beskytte gjenstandene dine</div>' );
	description.append( '<div class="escrow_explanation_a">Steam-kontoer er verdifulle, spesielt om de har gjenstander som er verdt å stjele. Hvis du ikke beskytter kontoen din med en fysisk enhet (mobilenhet for Steam Guard-autentisering), gir bytteoppbevaring deg tid til å oppdage om kontoen din har blitt kompromittert, og forhindre at gjenstander forsvinner fra kontoen din.</div>' );

	description.append( '<div class="escrow_explanation_q">En forsinkelse for å oppdage og stanse tyveri av gjenstander</div>' );
	description.append( '<div class="escrow_explanation_a">Hvis en bruker som bytter bort eller selger gjenstander, ikke har vært beskyttet av en mobilenhet for autentisering de siste 7 dagene: <br><span style="color: #5aa9d6">Byttehandel:</span> Leveransen av gjenstandene blir forsinket av Steam i opptil 15 dager. <br><span style="color: #5aa9d6">Marked:</span> Salgsannonser blir forsinket av Steam før de legges ut, i opptil 15 dager. <br><br>Dette gir brukeren tid til å avbryte ventende transaksjoner de ikke autoriserte. <br><br>Når du avbryter byttehandler som venter, eller er i oppbevaring, starter en tidsbegrenset sperre av byttehandel for kontoen din for å hindre fremtidige uautoriserte forsøk på å bytte bort gjenstander. Det er ingen sperre for avbrutte annonser på markedet.</div>' );

	description.append( '<div class="escrow_explanation_q">Fjern behovet for gjenstandsoppbevaring</div>' );
	description.append( '<div class="escrow_explanation_a">En mobilenhet for autentisering sikrer at du, og bare du, kan bytte bort eller selge gjenstandene dine fort og sikkert, slik at oppbevaring ikke er nødvendig lenger. Øk kontosikkerheten din ved å skaffe en <a href="https://store.steampowered.com/mobile/" target="_blank">mobilenhet for Steam Guard-autentisering</a> til iOS- og Android-enheter. </div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Les Steams blogginnlegg om kontosikkerhet</a> for en utfyllende forklaring på hvorfor vi implementerte oppbevaring.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Bytte- og markedsoppbevaring',
			description,
			'Les mer om mobilenheter for Steam Guard-autentisering',
			'Lukk'
		);
