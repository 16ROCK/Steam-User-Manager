...
function ConfirmFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'allow', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Bekreftelsesfeil',
					'Det oppstod et problem med denne handlingen. Prøv igjen senere.',
					'OK'
			);
		}
	);
...
function CancelFromDetails( confirmationID, confirmationKey )
{
	SendMobileConfirmationOp( 'cancel', confirmationID, confirmationKey,
		...
		function( data )
		{
			...
			ShowAlertDialog(
					'Bekreftelsesfeil',
					'Det oppstod et problem med å avbryte bekreftelsen. Prøv igjen senere.',
					'OK'
			);
		}
	);
...
$J( function() {
	...
	$J('.mobileconf_list_entry').click( function() {
		...
		GetValueFromLocalURL( 'steammobile://steamguard?op=conftag&arg1=details' + nConfirmationId, 5, function( httpParams )
		{
			$J.ajax( {
				...
				else
				{
					ShowAlertDialog(
							'Bekreftelsesfeil',
							typeof responseData.message != 'undefined' ? responseData.message : 'Det oppstod et problem med innlastingen av bekreftelsesdetaljene. Prøv igjen senere.',
							'OK'
					).always( function() {
						window.history.back();
					} );
				...
				ShowAlertDialog(
						'Bekreftelsesfeil',
						'Det oppstod et problem med innlastingen av bekreftelsesdetaljene. Prøv igjen senere.',
						'OK'
				).always( function() {
					window.history.back();
				} );
		...
		function( calldata ) {
			...
			ShowAlertDialog(
				'Bekreftelsesfeil',
				'Mobilappen kunne ikke laste inn detaljene om bekreftelsen. Start enheten på nytt og prøv forespørselen igjen senere. Hvis du stadig opplever et problem, kan det være at en annen enhet er blitt satt opp til å gi Steam Guard-kodene til kontoen, som betyr at denne autentiseringsenheten ikke lenger er gyldig.',
				'OK'
			).always( function() {
				window.history.back();
			} );
	...
	$J('.mobileconf_list_checkbox').click( function( e ) {
		...
		if ( nChecked > 0 )
		{
			...
			$btnCancel.text( 'Avbryt valgte' );
			$btnAccept.text( 'Bekreft valgte' );
