...
var CInventory = Class.create( {
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: '알 수 없는 항목 ' + itemid, descriptions: [], fraudwarnings: [ '이 항목에 대한 정보를 가져올 수 없습니다.' ] };
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( '모든 #{appname} 아이템');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( '이 보관함은 지금 사용할 수 없습니다. 나중에 다시 시도하십시오.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"\uc5ec\ub7ec\ubd84\uc740 \uac8c\uc784 \ud50c\ub808\uc774 \ub3c4\uc911 \ubb34\uc791\uc704\ub85c \uc5bb\uac70\ub098, \uac8c\uc784 \ub0b4\uc758 Mann Co. \uc0c1\uc810\uc5d0\uc11c \uad6c\ub9e4\ub97c \ud558\uac70\ub098, \uce5c\uad6c\uc640 \uac70\ub798\ub97c \ud568\uc73c\ub85c\uc368 \uc544\uc774\ud15c\uc744 \uc5bb\uc744 \uc218 \uc788\uc2b5\ub2c8\ub2e4."},"620":{"howtoget":"\ud3ec\ud138 2\uc758 \uac8c\uc784 \ub0b4 \uc0c1\uc810\uc774\ub098 \uce5c\uad6c\uc640\uc758 \uac70\ub798\ub97c \ud1b5\ud574 \uad6c\ud560 \uc218 \uc788\uc2b5\ub2c8\ub2e4."},"753":{"1":{"howtoget":"\uc5ec\ubd84\uc758 \ubcf5\uc0ac\ubcf8\uc740 Steam \uc0c1\uc810\uc5d0\uc11c \"\uc120\ubb3c\uc6a9\uc73c\ub85c \uad6c\ub9e4\"\ub97c \uc120\ud0dd\ud558\uc2dc\uac70\ub098 \ud2b9\ubcc4 \ud589\uc0ac \uae30\uac04\uc744 \ud1b5\ud574 \uc5bb\uc744 \uc218 \uc788\uc2b5\ub2c8\ub2e4."}},"99900":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.cloudflare.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> 로부터 #{contextname}');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( '이 금액은 비공개로 설정되었으며, 본인에게만 보입니다.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '보석 %s 개'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( '커뮤니티 장터에서 보기' );
		...
		elPriceInfoContent.update( '<img src="https://community.cloudflare.steamstatic.com/public/images/login/throbber.gif" alt="처리 중...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
				...
				onSuccess: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strInfo += '시작 가격: ' + transport.responseJSON.lowest_price + '<br>'
						}
						else
						{
							strInfo += '이 항목에 대한 목록이 없습니다.' + '<br>';
						}

						if ( transport.responseJSON.volume )
						{
							var strVolume = '지난 24시간 동안 %1$s개 판매됨';
							strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
							//strInfo += '평균 판매 가격: ' + transport.responseJSON.median_price + '<br>';
							strInfo += '판매량: ' + strVolume + '<br>';
						}
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', '판매' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'<a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">Steam 지갑에 자금을 추가</a>하거나, Steam 상점에서 아이템을 구매하고 대금 청구지 주소를 입력해야 장터에서 아이템을 판매할 수 있습니다.',
			'아이템을 판매할 수 없음'
		);
	}
	else
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( '아이템 판매하기' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( '이 아이템을 판매하려면 Steam 이용 약관 모든 조항에 동의해야 합니다.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( '유효한 양을 입력하십시오.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' 입력한 가격이 최고 가격 %1$s을(를) 넘습니다.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( '유효한 가격을 입력해야 합니다.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( '판매 가격 확인' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = '이 아이템을 Steam 장터에 등록하시려면 귀하의 Steam 모바일 앱으로 확인하셔야만 합니다. 앱을 실행하시고 메뉴에서 확인 페이지로 가서 확인하실 수 있습니다.' +
							'<br><br>' +
							'애플리케이션의 메인 메뉴에서 인증 페이지를 볼 수 없다면 애플리케이션 버전이 최신인지 확인해주세요.';

				}
				else
				{
					strText = '이 항목을 커뮤니티 장터 목록에 올리려면, 추가적인 확인 절차를 완료하셔야 합니다. 추가 지시 사항이 ("%s"로 끝나는) 귀하의 이메일 주소로 발송되었습니다.'.replace( /%s/, transport.responseJSON.email_domain );
				}
		...
		{
			this.DisplayError( '아이템을 올리는데 문제가 있습니다. 페이지를 새로 고친 후 다시 시도하십시오.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( '선택 사항을 저장하는 중에 문제가 발생했습니다. 잠시 후에 다시 시도해 주세요.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: '평균 판매 가격', textAlign: 'left' },
		...
		highlighter: {
			...
			fadeTooltip: true,
            yvalues: 2,
            formatString: '<strong>%s</strong><br>%s<br>%d개 판매됨'
		},
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Steam 고객지원에 %s님을 거래 사기 시도로 신고하는 데 사용하세요.<br><br>간략한 설명도 기재해 주세요.'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( '사기 신고', $Dialog, '사기 신고').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( '사기 신고', '설명이 필요합니다.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( '감사합니다!', '고객님의 신고가 접수되었으며, Steam 고객지원에서 검토할 예정입니다.' );
		}).fail( function() {
			ShowAlertDialog( '사기 신고', '귀하의 보고를 저장하는 데 문제가 발생하였습니다. 나중에 다시 시도해 주세요.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">거래 및 장터 지연이 무엇인가요?</div>' );
	description.append( '<div class="escrow_explanation_a">지연은 거래가 완료되기 전이나 장터 목록을 올리기 전에 Steam에서 일시적으로 아이템을 맡아두는 기간입니다.</div>' );

	description.append( '<div class="escrow_explanation_q">아이템 지연이 여러분의 아이템을 보호하는 데 도움을 줍니다.</div>' );
	description.append( '<div class="escrow_explanation_a">Steam 계정은 소중하며, 특히 도난당할 만한 아이템을 소지하고 있을 때는 더더욱 그렇습니다. 귀하의 계정을 물리적 장치(Steam Guard 모바일 인증기)로 보호하지 않았다면 거래 지연이 적용되어 자신의 계정이 손상되었다는 사실을 인지하고 계정에서 아이템이 빠져나가는 것을 방지할 시간을 제공합니다.</div>' );

	description.append( '<div class="escrow_explanation_q">아이템 도난방지를 위한 지연 기간</div>' );
	description.append( '<div class="escrow_explanation_a">계정을 모바일 인증기로 보호한 지 아직 7일이 지나지 않은 상태에서 아이템을 거래 또는 판매하는 경우:<br><span style="color: #5aa9d6">거래:</span> 거래 완료 후 최대 15일 동안 Steam에서 아이템 지급을 보류합니다.<br><span style="color: #5aa9d6">장터:</span> 장터에 아이템을 올리면 Steam에서 최대 15일 동안 보류했다가 장터에 게시합니다.<br><br>다른 사람이 내 허락 없이 내 아이템을 거래하려고 할 때 이를 취소할 수 있도록 시간을 주는 것입니다.<br><br>대기 중이거나 보류 중인 거래를 취소하면 일정 기간 거래가 불가능합니다. 이는 사용자 허락 없이 아이템을 추가로 거래할 수 없도록 막기 위함입니다. 장터에 올린 아이템을 취소할 때는 이와 같은 제한이 없습니다.</div>' );

	description.append( '<div class="escrow_explanation_q">아이템 지연 제거하기</div>' );
	description.append( '<div class="escrow_explanation_a">모바일 인증기를 사용하면 본인 이외의 다른 사람이 계정에 접근하는 것을 차단하므로 지연없이 빠르고 안전하게 아이템을 거래하고 판매할 수 있습니다. iOS와 Android 기기에서 사용할 수 있는 <a href="https://store.steampowered.com/mobile/" target="_blank">Steam Guard 모바일 인증기</a>로 계정의 보안을 강화하세요.</div>' );

	description.append( '<div class="escrow_explanation_a">Steam에 거래 지연이 도입된 이유에 대한 자세한 설명은 <a href="http://store.steampowered.com/news/20631/" target="_blank">계정 보안에 대한 Steam 블로그 게시글</a>에서 확인하실 수 있습니다.</div>' );
	...
	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'거래 및 장터 지연',
			description,
			'Steam Guard 모바일 인증기에 대해 자세히 알아보기',
			'닫기'
		);
